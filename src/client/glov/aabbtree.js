...
AABBTree.allocateNode = function () {
  let nodesPool = this.nodesPool;
  if (!nodesPool.length) {
    // Allocate a bunch of nodes in one go
    let nodesPoolAllocationSize = this.nodesPoolAllocationSize;
    let extentsArray = new Float32Array(nodesPoolAllocationSize * 6);
    let extentsArrayIndex = 0;
    for (let n = 0; n < nodesPoolAllocationSize; n += 1) {
...
AABBTree.prototype.rebuild = function () {
  if (this.numExternalNodes > 0) {
    ...
    if (this.numExternalNodes === nodes.length) {
      ...
      for (let n = 0; n < endNodeIndex; n += 1) {
    ...
    if (numBuildNodes > 1) {
      ...
      rootNode = nodes[0];
      let extents = rootNode.extents;
      let deltaX = (extents[3] - extents[0]);
      let deltaY = (extents[4] - extents[1]);
      let deltaZ = (extents[5] - extents[2]);
...
AABBTree.prototype._sortNodes = function (nodes) {
  let numNodesLeaf = this.numNodesLeaf;
  let numNodes = nodes.length;
  ...
  let nthElement = this._nthElement;
  let reverse = false;
...
AABBTree.prototype._sortNodesHighQuality = function (nodes) {
  ...
  let nthElement = this._nthElement;
  let calculateSAH = this._calculateSAH;
  let reverse = false;
...
AABBTree.prototype._calculateSAH = function (buildNodes, startIndex, endIndex) {
  ...
  for (let n = (startIndex + 1); n < endIndex; n += 1) {
...
export function create(highQuality) {
  return new AABBTree(Boolean(highQuality));
}
