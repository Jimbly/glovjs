...
"use strict"
window.glov_build_version="1659041849443"
...
if(input.mouseDown(bounds)){v4copy(test.color_sprite,color_yellow)}else if(input.click(bounds)){v4copy(test.color_sprite,test.color_sprite[2]===0?color_white:color_red)
...
if(input.mouseDown(bounds)){v4copy(_test.color_sprite,color_yellow)}else if(input.click(bounds)){v4copy(_test.color_sprite,_test.color_sprite[2]===0?color_white:color_red)
...
dst[1]=(src[1]-data[1])*data[5]}function canvasToVirtual(dst,src){dst[0]=src[0]/data[4]+data[0]
...
if(clip_pause){clipResume()}if(input.click({peek:true})||!handled&&input.mouseDown({peek:true})){state.open=false}}v3copy(param.color,state.rgba)}
...
function errorReportSetDetails(key,value){if(value){error_report_details[key]=escape(String(value))}else{delete error_report_details[key]}}function errorReportSetDynamicDetails(key,fn){error_report_dynamic_details[key]=fn}errorReportSetDetails("build","1659041849443")
...
var engine=require("./engine.js")
var geom=require("./geom.js")
var _require=require("./localization.js"),getStringFromLocalizable=_require.getStringFromLocalizable
var max=Math.max,round=Math.round
var shaders=require("./shaders.js")
var sprites=require("./sprites.js")
var BLEND_ALPHA=sprites.BLEND_ALPHA,BLEND_PREMULALPHA=sprites.BLEND_PREMULALPHA,spriteChainedStart=sprites.spriteChainedStart,spriteChainedStop=sprites.spriteChainedStop,queueraw=sprites.queueraw
var textures=require("./textures.js")
var _require2=require("../common/util.js"),clamp=_require2.clamp
var _require3=require("../common/vmath.js"),v3scale=_require3.v3scale,vec4=_require3.vec4,v4clone=_require3.v4clone,v4copy=_require3.v4copy,v4scale=_require3.v4scale
...
var temp_color=vec4()
var geom_stats
function createTechniqueParameters(){if(tech_params){return}geom_stats=geom.stats
tech_params={param0:vec4(),outline_color:vec4(),glow_color:vec4(),glow_params:vec4()}}function techParamsSet(param,value){var tpv=tech_params[param]
if(!tech_params_dirty){if(tpv[0]!==value[0]||tpv[1]!==value[1]||tpv[2]!==value[2]||tpv[3]!==value[3]){tech_params={param0:v4clone(tech_params.param0),outline_color:v4clone(tech_params.outline_color),glow_color:v4clone(tech_params.glow_color),glow_params:v4clone(tech_params.glow_params)}
geom_stats.font_params++
tech_params_dirty=true
tpv=tech_params[param]}else{return}}if(tech_params_dirty){tpv[0]=value[0]
tpv[1]=value[1]
tpv[2]=value[2]
tpv[3]=value[3]}}function sameTP(as){for(var key in tech_params){var v1=tech_params[key]
var v2=as[key]
...
for(var ii=0;ii<tech_params_cache.length;++ii){if(sameTP(tech_params_cache[ii])){tech_params=tech_params_cache[ii]
...
_char_info.yoffs=_char_info.yoffs||0}this.replacement_character=this.infoFromChar(65533)
...
createTechniqueParameters()}GlovFont.prototype.drawSizedColor=function(style,x,y,z,size,color,text){return this.drawSized(fontStyleColored(style,color),x,y,z,size,text)}
GlovFont.prototype.drawSized=function(style,x,y,z,size,text){return this.drawScaled(style,x,y,z,size/this.font_info.font_size,size/this.font_info.font_size,text)}
GlovFont.prototype.drawSizedAligned=function(style,_x,_y,z,size,align,w,h,text){profilerStart("drawSizedAligned")
text=getStringFromLocalizable(text)
if(align&ALIGN.HWRAP){var drawn_height=this.drawSizedAlignedWrapped(style,_x,_y,z,0,size,align&~ALIGN.HWRAP,w,h,text)
profilerStop("drawSizedAligned")
return drawn_height}var x_size=size
var y_size=size
var width=this.getStringWidth(style,x_size,text)
if(align&ALIGN.HFIT&&width>w){var scale=w/width
x_size*=scale
width=w
if(scale<.5){if((align&ALIGN.VMASK)!==ALIGN.VCENTER&&(align&ALIGN.VMASK)!==ALIGN.VBOTTOM){_y+=(y_size-y_size*scale*2)/2}y_size*=scale*2}}var height=y_size
var x
var y
switch(align&ALIGN.HMASK){case ALIGN.HCENTER:x=_x+(w-width)/2
if(this.font_info.noFilter){x|=0}break
case ALIGN.HRIGHT:x=_x+w-width
break
case ALIGN.HLEFT:x=_x
break
default:x=_x}switch(align&ALIGN.VMASK){case ALIGN.VCENTER:y=_y+(h-height)/2
if(this.font_info.noFilter){y|=0}break
case ALIGN.VBOTTOM:y=_y+h-height
break
case ALIGN.VTOP:y=_y
break
default:y=_y}var drawn_width=this.drawScaled(style,x,y,z,x_size/this.font_info.font_size,y_size/this.font_info.font_size,text)
profilerStop("drawSizedAligned")
return drawn_width}
...
if(this.font_info.noFilter){yoffs|=0}break
...
GlovFont.prototype.drawSizedColorWrapped=function(style,x,y,z,w,indent,size,color,text){return this.drawScaledWrapped(fontStyleColored(style,color),x,y,z,w,indent,size/this.font_info.font_size,size/this.font_info.font_size,text)}
GlovFont.prototype.drawSizedWrapped=function(style,x,y,z,w,indent,size,text){return this.drawScaledWrapped(style,x,y,z,w,indent,size/this.font_info.font_size,size/this.font_info.font_size,text)}
var default_size=24
...
var xsc=x_size/this.font_info.font_size
...
var char_info=this.infoFromChar(c)
if(char_info){ret+=(char_info.w+char_info.xpad)*xsc*char_info.scale+x_advance}}return ret}
...
if(char_info){var char_w=(char_info.w+char_info.xpad)*xsc*char_info.scale+x_advance
word_w+=char_w
...
this.applyStyle(style)
this.last_width=0
var num_lines=this.wrapLinesScaled(w,indent,xsc,text,0,function(xoffs,linenum,line,x1){var y2=y+_this.font_info.font_size*ysc*linenum
var x2=x+xoffs
_this.drawScaled(style,x2,y2,z,xsc,ysc,line)
_this.last_width=max(_this.last_width,x1)})
return num_lines*this.font_info.font_size*ysc}
GlovFont.prototype.calcXAdvance=function(xsc){var font_texel_scale=this.font_info.font_size/32
...
return x_advance}
GlovFont.prototype.drawScaled=function(style,_x,y,z,xsc,ysc,text){profilerStart("drawScaled")
text=getStringFromLocalizable(text)
var x=_x
assert(isFinite(x))
assert(isFinite(y))
...
var x_advance=this.calcXAdvance(xsc)
var font_texel_scale=font_info.font_size/32
var tile_state=0
...
var value=vec4(1/delta_per_dest_pixel,-.5/delta_per_dest_pixel+.5,-.5/delta_per_dest_pixel+.5+applied_style.outline_width*font_texel_scale*avg_scale_combined,0)
if(value[2]>0){value[2]=0}var padding1=max(0,applied_style.outline_width*font_texel_scale*avg_scale_font)
var padding4=vec4()
...
var value2=vec4(0,0,1/((applied_style.glow_outer-applied_style.glow_inner)*delta_per_source_pixel*font_texel_scale),-(.5-applied_style.glow_outer*delta_per_source_pixel*font_texel_scale)/((applied_style.glow_outer-applied_style.glow_inner)*delta_per_source_pixel*font_texel_scale))
if(value2[3]>0){value2[3]=0}var padding_in_font_space=vec4()
...
var sort_y=(y-camera2d.data[1])*camera2d.data[5]
for(var i=0;i<len;i++){var c=text.charCodeAt(i)
if(c===9){var tabsize=xsc*font_info.font_size*4
...
var elem=queueraw(texs,xx,yy,z+z_advance*i,w,h,u0,v0,u1,v1,applied_style.color_vec4,this.shader,tech_params,blend_mode)
elem.y=sort_y}x+=(char_info.w+char_info.xpad)*xsc2+x_advance}}}if(!z_advance){spriteChainedStop()}profilerStop("drawScaled")
return x-_x}
...
exports.PAD=PAD
var _require=require("./browser.js"),is_firefox=_require.is_firefox,is_mac_osx=_require.is_mac_osx
var camera2d=require("./camera2d.js")
var _require2=require("./cmds.js"),cmd_parse=_require2.cmd_parse
var engine=require("./engine.js")
...
var settings=require("./settings.js")
var _require3=require("./sound.js"),soundResume=_require3.soundResume
var _require4=require("./spot.js"),spotMouseoverHook=_require4.spotMouseoverHook,BUTTON_ANY=_require4.BUTTON_ANY
var _require5=require("../common/vmath.js"),vec2=_require5.vec2,v2add=_require5.v2add,v2copy=_require5.v2copy,v2lengthSq=_require5.v2lengthSq,v2set=_require5.v2set,v2scale=_require5.v2scale,v2sub=_require5.v2sub
...
in_event.topOfFrame()
ctrl_checked=false
if(touches[pointerlock_touch_id]&&!pointerLocked()){pointerLockExit()}}function endFrameTickMap(map){Object.keys(map).forEach(function(keycode){switch(map[keycode]){case DOWN_EDGE:map[keycode]=DOWN
break
...
return dst}function mouseDomPos(){return mouse_pos}function mouseMoved(){return mouse_moved}function mouseButtonHadEdge(){return mouse_button_had_edge}function mouseButtonHadUpEdge(){return mouse_button_had_up_edge}function mousePosParam(param){param=param||{}
return{x:param.x===undefined?camera2d.x0Real():param.x,y:param.y===undefined?camera2d.y0Real():param.y,w:param.w===undefined?camera2d.wReal():param.w,h:param.h===undefined?camera2d.hReal():param.h,button:param.button===undefined?ANY:param.button}}var check_pos=vec2()
...
return ret}function mouseDown(param){if(input_eaten_mouse){return null}param=param||{}
var pos_param=mousePosParam(param)
var button=pos_param.button
var max_click_dist=Infinity
if(param.do_max_dist){max_click_dist=param.max_dist||50}for(var touch_id in touches){var touch_data=touches[touch_id]
if(touch_data.state!==DOWN||!(button===ANY||button===touch_data.button)||touch_data.total>max_click_dist){continue}if(checkPos(touch_data.cur_pos,pos_param)){return{button:touch_data.button,pos:check_pos.slice(0),start_time:touch_data.start_time}}}return null}function mousePosIsTouch(){return mouse_pos_is_touch}function numTouches(){return Object.keys(touches).length}function keyDown(keycode){if(keycode===KEYS.CTRL){ctrl_checked=true}if(input_eaten_kb){return 0}var ks=key_state_new[keycode]
...
in_event.on("mouseup",param.phys,param.in_event_cb)}return false}function mouseDownEdge(param){param=param||{}
var pos_param=mousePosParam(param)
var button=pos_param.button
for(var touch_id in touches){var touch_data=touches[touch_id]
...
camera2d.virtualToDomPosParam(param.phys,pos_param)
in_event.on("mousedown",param.phys,param.in_event_cb)}return false}function mouseConsumeClicks(param){param=param||{}
var pos_param=mousePosParam(param)
var button=pos_param.button
for(var touch_id in touches){var touch_data=touches[touch_id]
...
touch_data.total=Infinity}}}function drag(param){param=param||{}
var pos_param=mousePosParam(param)
var button=pos_param.button
...
return{cur_pos:cur_pos,start_pos:start_pos,delta:delta,total:total,button:touch_data.button,touch:touch_data.touch,start_time:touch_data.start_time,is_down_edge:is_down_edge,down_time:touch_data.down_time}}}return null}function longPress(param){param=param||{}
var pos_param=mousePosParam(param)
var button=pos_param.button
...
camera2d.domToVirtual(cur_pos,touch_data.cur_pos)
camera2d.domDeltaToVirtual(delta,touch_data.delta)
return{long_press:true,cur_pos:cur_pos,start_pos:start_pos,delta:delta,total:total,button:touch_data.button,touch:touch_data.touch,start_time:touch_data.start_time,is_down_edge:is_down_edge,down_time:touch_data.down_time}}}return null}function dragDrop(param){param=param||{}
var pos_param=mousePosParam(param)
var button=pos_param.button
for(var touch_id in touches){var touch_data=touches[touch_id]
...
touch_data.dispatched=true}return{drag_payload:touch_data.drag_payload}}}return null}function dragOver(param){param=param||{}
var pos_param=mousePosParam(param)
var button=pos_param.button
for(var touch_id in touches){var touch_data=touches[touch_id]
...
},{"../common/vmath.js":91,"./browser.js":11,"./camera2d.js":13,"./cmds.js":16,"./engine.js":20,"./in_event.js":35,"./local_storage.js":38,"./pointer_lock.js":47,"./settings.js":55,"./sound.js":61,"./spot.js":63,"assert":undefined}],37:[function(require,module,exports){
"use strict"
exports.link=link
...
subs.tick(dt)})}}var build_timestamp_string=new Date(Number("1659041849443")).toISOString().replace("T"," ").slice(5,-8)
...
for(var _ii=0;_ii<HIST_TOT;_ii+=HIST_COMPONENTS){count_sum+=walk.history[_ii]}if(!count_sum){return true}if(input.mouseOver({x:0,y:y,w:line_width,h:LINE_HEIGHT,peek:true})){mouseover_main_elem=walk
...
walk=walk.next}return count}function profilerShowEntry(walk,depth){if(settings.profiler_relative===0&&walk===node_out_of_tick){return false}var time_sum=0
var count_sum=0
var time_max=0
var sum_count=0
...
if(do_ui){if(input.click({x:0,y:y,w:line_width,h:LINE_HEIGHT,button:0})){walk.toggleShowChildren()}else if(input.click({x:0,y:y,w:line_width,h:LINE_HEIGHT,button:1})){walk.parent.toggleShowChildren()}}profilerStart("bar graph")
spriteChainedStart()
...
function profilerUIRun(){profilerStart("profilerUIRun")
...
y=0
var x=line_width
if(ui.buttonText({x:x,y:y,z:z,w:BUTTON_W,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:settings.profiler_interactable?"interactable":"overlay"})){settings.set("profiler_interactable",1-settings.profiler_interactable)}do_ui=settings.profiler_interactable
if(do_ui&&ui.buttonText({x:x+BUTTON_W,y:y,z:z,w:BUTTON_H,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:"X"})){settings.set("show_profiler",0)}y+=BUTTON_H
var text=loaded_profile?"loaded":profilerPaused()?"paused":"live"
if(do_ui){if(ui.buttonText({x:x,y:y,z:z,w:BUTTON_W,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:text})){if(loaded_profile){useLiveProfile()}else{profilerPause(!profilerPaused())}}}else{font.drawSizedAligned(null,x,y,z,FONT_SIZE,font.ALIGN.HVCENTERFIT,BUTTON_W,BUTTON_H,text)}y+=BUTTON_H
if(do_ui){if(ui.buttonText({x:x,y:y,z:z,w:BUTTON_W,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:PROFILER_RELATIVE_LABELS[settings.profiler_relative]})){settings.set("profiler_relative",(settings.profiler_relative+1)%PROFILER_RELATIVE_LABELS.length)}}else{font.drawSizedAligned(null,x,y,z,FONT_SIZE,font.ALIGN.HVCENTERFIT,BUTTON_W,BUTTON_H,PROFILER_RELATIVE_LABELS[settings.profiler_relative])}y+=BUTTON_H
if(do_ui){if(ui.buttonText({x:x,y:y,z:z,w:BUTTON_W,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:settings.profiler_average?"average":"last frame"})){settings.set("profiler_average",1-settings.profiler_average)}}else{font.drawSizedAligned(null,x,y,z,FONT_SIZE,font.ALIGN.HVCENTERFIT,BUTTON_W,BUTTON_H,settings.profiler_average?"average":"last frame")}y+=BUTTON_H
if(do_ui){if(ui.buttonText({x:x,y:y,z:z,w:BUTTON_W,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:settings.profiler_graph?"graph: mem":"graph: CPU"})){settings.set("profiler_graph",1-settings.profiler_graph)}}else{font.drawSizedAligned(null,x,y,z,FONT_SIZE,font.ALIGN.HVCENTERFIT,BUTTON_W,BUTTON_H,settings.profiler_graph?"graph: mem":"graph: CPU")}y+=BUTTON_H
...
y+=LINE_HEIGHT
if(do_ui){if(ui.buttonText({x:x,y:y,z:z,w:BUTTON_W/3,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:"-",disabled:loaded_profile||cur_depth===0})){profilerMemDepthSet(cur_depth-1)
settings.set("profiler_mem_depth",profilerMemDepthGet())}if(ui.buttonText({x:x+BUTTON_W/3,y:y,z:z,w:BUTTON_W/3,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:""+(cur_depth||"OFF"),disabled:loaded_profile})){if(cur_depth===MEM_DEPTH_DEFAULT){profilerMemDepthSet(99)}else{profilerMemDepthSet(MEM_DEPTH_DEFAULT)}settings.set("profiler_mem_depth",profilerMemDepthGet())}if(ui.buttonText({x:x+2*BUTTON_W/3,y:y,z:z,w:BUTTON_W/3,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:"+",disabled:loaded_profile})){profilerMemDepthSet(cur_depth+1)
settings.set("profiler_mem_depth",profilerMemDepthGet())}}else{font.drawSizedAligned(null,x,y,z,FONT_SIZE,font.ALIGN.HVCENTERFIT,BUTTON_W,BUTTON_H,""+(cur_depth||"OFF"))}y+=BUTTON_H}text=settings.max_fps===1e3?"max CPU":settings.max_fps===0?"anim frame":"?"
if(do_ui){if(ui.buttonText({x:x,y:y,z:z,w:BUTTON_W,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:text})){settings.set("max_fps",settings.max_fps===0?1e3:0)}}else{font.drawSizedAligned(null,x,y,z,FONT_SIZE,font.ALIGN.HVCENTERFIT,BUTTON_W,BUTTON_H,text)}y+=BUTTON_H
...
y+=LINE_HEIGHT
if(do_ui){if(ui.buttonText({x:x,y:y,z:z,w:BUTTON_W/2,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:"save",disabled:loaded_profile})){var a=document.createElement("a")
a.href="data:application/json,"+encodeURIComponent(profilerExport())
a.setAttribute("download","profile.json")
a.click()}if(ui.buttonText({x:x+BUTTON_W/2,y:y,z:z,w:BUTTON_W/2,h:BUTTON_H,font_height:BUTTON_FONT_HEIGHT,text:"load"})){var input_elem=document.createElement("input")
...
this.consumed_click=true}else if(!input.mouseDown({button:0})){this.grabbed=false
...
var mouseDown=_inputJs.mouseDown
...
var pad_mode=false
function spotPadMode(){return pad_mode}function spotlog(){}function spotKey(param){profilerStart("spotKey")
...
focused=true
if(mouseDown()){state=SPOT_STATE_DOWN}}else if(drag_over&&dragOver(param)){}}if(allow_focus&&inputTouchMode()){var long_press_focuses=param.long_press_focuses===undefined?def.long_press_focuses:param.long_press_focuses
...
if(focus_key===param.key_computed){spotUnfocus()}else if(focus_key_nonsticky===param.key_computed){focus_key_nonsticky=null}}}if(is_mouseover){if(allow_focus){if(spotFocusSet(param,true,false,"mouseover")){focused=true}}}if(is_button&&mouseDown({x:param.x,y:param.y,w:param.w,h:param.h,do_max_dist:true})){if(!disabled){state=SPOT_STATE_DOWN}}var button_activate=false
...
exports.queuesprite4color=queuesprite4color
...
temp_uvs[3]=uvs[3]-vbias/tex.height}function queuesprite4color(sprite,x,y,z,w,h,rot,uvs,c0,c1,c2,c3,shader,shader_params,nozoom,pixel_perfect,blend){assert(isFinite(z))
var elem=spriteDataAlloc(sprite.texs,shader,shader_params,blend)
x=(x-camera2d.data[0])*camera2d.data[4]
y=(y-camera2d.data[1])*camera2d.data[5]
w*=camera2d.data[4]
h*=camera2d.data[5]
if(pixel_perfect){x|=0
y|=0
w|=0
h|=0}elem.x=x
elem.y=y
...
data[17]=_y+sw+ch
data[24]=_x+cw
data[25]=_y+sw}fillUVs(elem.texs[0],w,h,nozoom,uvs)
data[2]=c0[0]
data[3]=c0[1]
data[4]=c0[2]
data[5]=c0[3]
...
return queuesprite4color(sprite,x,y,z,w,h,rot,uvs,color,color,color,color,shader,shader_params,nozoom,pixel_perfect,blend)}var clip_temp_xy=vec2()
...
return queuesprite(this,params.x,params.y,params.z||Z.UI,w,h,params.rot,uvs,params.color||this.color,params.shader||this.shader,params.shader_params,params.nozoom,params.pixel_perfect,params.blend)}
...
var h=(params.h||1)*this.size[1]
var uvs=typeof params.frame==="number"?this.uidata.rects[params.frame]:params.uvs||this.uvs
return queuesprite4color(this,params.x,params.y,params.z||Z.UI,w,h,params.rot,uvs,params.color_ul,params.color_ll,params.color_lr,params.color_ur,params.shader||this.shader,params.shader_params,params.nozoom,params.pixel_perfect,params.blend)}
...
modalTextEntry({edit_w:400,edit_text:str.replace(/[\n\r]/g," "),title:title,text:copy_success?default_copy_success_msg:default_copy_failure_msg,buttons:{ok:null}})}function drawRect(x0,y0,x1,y1,z,color){var mx=min(x0,x1)
var my=min(y0,y1)
var Mx=max(x0,x1)
var My=max(y0,y1)
return sprites.white.draw({x:mx,y:my,z:z,color:color,w:Mx-mx,h:My-my})}function drawRect2(param){return drawRect(param.x,param.y,param.x+param.w,param.y+param.h,param.z,param.color)}function drawRect4Color(x0,y0,x1,y1,z,color_ul,color_ur,color_ll,color_lr){var mx=min(x0,x1)
var my=min(y0,y1)
var Mx=max(x0,x1)
var My=max(y0,y1)
return sprites.white.draw4Color({x:mx,y:my,z:z,color_ul:color_ul,color_ll:color_ll,color_lr:color_lr,color_ur:color_ur,w:Mx-mx,h:My-my})}function spreadTechParams(spread){spread=min(max(spread,0),.99)
...
function getVersionUrlParams(){return"plat="+PLATFORM+"&ver="+exports.CURRENT_VERSION+"&build="+"1659041849443"}function jsonParseResponse(response){if(!response){return null}if(response.trim()[0]==="<"){return null}try{return JSON.parse(response)}catch(e){return null}}function whenServerReady(cb){var retry_count=0
...
return}}cb()})}doit()}WSClient.prototype.onBuildTimestamp=function(build_timestamp){if(build_timestamp!=="1659041849443"){if(this.on_build_timestamp_mismatch){this.on_build_timestamp_mismatch()}else{if(getAbilityReload()){console.error("App build mismatch (server: "+build_timestamp+", client: "+"1659041849443"+"), reloading")
whenServerReady(function(){if(window.reloadSafe){window.reloadSafe()}else{document.location.reload()}})}else{console.warn("App build mismatch (server: "+build_timestamp+", client: "+"1659041849443"+"), ignoring")}}}}
