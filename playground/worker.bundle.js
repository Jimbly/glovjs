...
if((evt=evt.data)instanceof Object&&evt.id){assert(handlers[evt.id])
try{handlers[evt.id](evt.data)}catch(e){sendmsg("error",{message:e.message||String(e),stack:e.stack})}}else console.log("worker (worker thread) unhandled message",evt)
endWork()}
