...
function formatUserID(user_id,display_name){if(user_id.match(guest_regex))user_id="guest"
var name=display_name||user_id
if(user_id.toLowerCase()!==name.toLowerCase())name=display_name+" ("+user_id+")"
return name}function AccountUI(){this.edit_box_name=ui.createEditBox({placeholder:"Username",initial_focus:true,text:local_storage.get("name")||"",autocomplete:"username"})
...
if(use_name&&(local_storage.get("name")||"").match(guest_regex))name=local_storage.get("name")
else name="anon"+String(random()).slice(2,8)
var pass="test"
local_storage.set("name",name)
this.edit_box_name.setText(name)
net.subs.login(name,pass,function(err){if(err)ui.modalDialog({title:"Auto-login Failed",text:err,buttons:{Retry:function Retry(){local_storage.set("did_auto_anon",void 0)
local_storage.set("name",void 0)},Cancel:null}})
else net.subs.sendCmdParse("rename_random",function(err){if(err)console.log(err)})})}
AccountUI.prototype.showLogin=function(param){var _this=this
var x=param.x,y=param.y,style=param.style,button_height=param.button_height,button_width=param.button_width,prelogout=param.prelogout,prelogin=param.prelogin,center=param.center,url_tos=param.url_tos,url_priv=param.url_priv,text_w=param.text_w,font_height=param.font_height,font_height_small=param.font_height_small,label_w=param.label_w,pad=param.pad,status_bar=param.status_bar
font_height=font_height||ui.font_height
font_height_small=font_height_small||.75*font_height
button_height=button_height||ui.button_height
button_width=button_width||240
text_w=text_w||400
label_w=label_w||round(140*font_height/24)
pad=pad||10
...
var min_h=2*BOX_H+3*pad+button_height
...
y+=BOX_H+pad}}if(!net.client.connected)login_message="Establishing connection..."
else if(net.subs.logging_in)login_message="Logging in..."
else if(net.subs.logging_out)login_message="Logging out..."
else if(!net.subs.loggedIn()&&window.FBInstant)net.subs.loginFacebook(function(err){if(err)ui.modalDialog({title:"Facebook login Failed",text:err,buttons:{Cancel:null}})})
else if(!net.subs.loggedIn()&&net.subs.auto_create_user&&!local_storage.get("did_auto_anon")&&!local_storage.get("name")){login_message="Creating guest account..."
...
if(ui.buttonText({x:x+w+(center?0:label_w)+pad,y:y,w:.5*button_width,h:BOX_H+pad-2,font_height:font_height_small,text:"Random"}))net.client.send("random_name",null,function(ignored,data){if(data)edit_box_display_name.setText(data)})
y+=BOX_H+pad
showTOS(true)
submit=ui.buttonText({x:x,y:y,w:button_width,h:button_height,font_height:font_height,text:"Create User"})||submit
if(ui.buttonText({x:x+button_width+pad,y:y,w:button_width,h:button_height,font_height:font_height,text:"Cancel"})||keyDownEdge(KEYS.ESC))this.creation_mode=false
y+=button_height+pad
if(submit){local_storage.set("name",edit_box_name.text)
net.subs.userCreate({user_id:edit_box_name.text,email:edit_box_email.text,password:edit_box_password.text,password_confirm:edit_box_password_confirm.text,display_name:edit_box_display_name.text},function(err){if(err)ui.modalDialog({title:"Login Error",text:err,buttons:{OK:null}})
else{_this.creation_mode=false
...
if(center)y+=button_height+pad
if(ui.buttonText({x:center?x:x+button_width+pad,y:y,w:button_width,h:button_height,font_height:font_height,text:"New User"})){this.creation_mode=true
edit_box_display_name.setText(edit_box_name.text)
if(edit_box_name.text&&edit_box_password.text)edit_box_password_confirm.initial_focus=true
else{edit_box_password_confirm.initial_focus=false
...
var name=formatUserID(user_id,net.subs.getChannel("user."+user_id).getChannelData("public.display_name")||user_id)
...
if(click({x:x-text_w/2,y:y,w:text_w,h:logged_in_font_height,button:0}))ui.provideUserString("Your User ID",user_id)
y+=logged_in_font_height+8}else if(status_bar){ui.font.drawSizedAligned(style,x+button_width+8,y,Z.UI,logged_in_font_height,glov_font.ALIGN.HFIT|glov_font.ALIGN.VCENTER,text_w,button_height,"Logged in as: "+name)
if(click({x:x+button_width+8,y:y,w:text_w,h:button_height,button:0}))ui.provideUserString("Your User ID",user_id)}else{ui.font.drawSizedAligned(style,x+button_width+8,y+-.25*logged_in_font_height,Z.UI,logged_in_font_height,calign|glov_font.ALIGN.VCENTER|glov_font.ALIGN.HFIT,text_w,button_height,"Logged in as:")
ui.font.drawSizedAligned(style,x+button_width+8,y+.75*logged_in_font_height,Z.UI,logged_in_font_height,calign|glov_font.ALIGN.VCENTER|glov_font.ALIGN.HFIT,text_w,button_height,name)
if(click({x:x+button_width+8,y:y,w:text_w,h:2*logged_in_font_height,button:0}))ui.provideUserString("Your User ID",user_id)}if(ui.buttonText({x:center?x-button_width/2:x,y:y,w:button_width,h:button_height,font_height:font_height,text:"Log out"})){if(prelogout)prelogout()
this.logout()}y+=button_height+8}else ui.font.drawSizedAligned(style,center?x-text_w/2:x+button_width+8,y,Z.UI,font_height,(center?glov_font.ALIGN.HCENTER:calign)|glov_font.ALIGN.VCENTER|glov_font.ALIGN.HFIT,text_w,button_height,"Logged in as: "+name)}if(login_message){var _w=ui.font.drawSizedAligned(style,center?x-400:x,y,Z.UI,1.5*font_height,glov_font.ALIGN.HVCENTERFIT,center?800:400,min_h,login_message)
_w+=100
...
},{"../glov/client/font.js":32,"../glov/client/input.js":42,"../glov/client/link.js":44,"../glov/client/local_storage.js":45,"../glov/client/net.js":50,"../glov/client/ui.js":78,"../glov/common/vmath.js":101,"assert":undefined}],2:[function(require,module,exports){
"use strict"
window.glov_build_version="1673321730282"
var called_once=false
window.onload=function(){if(called_once)return
called_once=true
require("../glov/client/bootstrap.js")
if("multiplayer"===window.conf_env)require("./multiplayer.js").main()
else if("entity"===window.conf_env)require("./enttest.js").main()
else require("./main.js").main()}

},{"../glov/client/bootstrap.js":12,"./enttest.js":3,"./main.js":7,"./multiplayer.js":8}],3:[function(require,module,exports){
"use strict"
exports.main=main
function _inheritsLoose(subClass,superClass){subClass.prototype=Object.create(superClass.prototype)
_setPrototypeOf(subClass.prototype.constructor=subClass,superClass)}function _setPrototypeOf(o,p){return(_setPrototypeOf=Object.setPrototypeOf?Object.setPrototypeOf.bind():function _setPrototypeOf(o,p){o.__proto__=p
return o})(o,p)}var _glovClientLocal_storage=require("../glov/client/local_storage")
var local_storage=_glovClientLocal_storage
_glovClientLocal_storage.setStoragePrefix("glovjs-multiplayer")
var assert=require("assert")
var _glovClientChat_ui=require("../glov/client/chat_ui")
var chatUICreate=_glovClientChat_ui.chatUICreate
...
var _glovClientEntity_base_client=require("../glov/client/entity_base_client")
var EntityBaseClient=_glovClientEntity_base_client.EntityBaseClient
var _glovClientEntity_manager_client=require("../glov/client/entity_manager_client")
var clientEntityManagerCreate=_glovClientEntity_manager_client.clientEntityManagerCreate
var _glovClientEntity_position_manager=require("../glov/client/entity_position_manager")
var entityPositionManagerCreate=_glovClientEntity_position_manager.entityPositionManagerCreate
var _glovClientFont=require("../glov/client/font")
var glov_font=_glovClientFont
...
var _glovClientNet2=require("../glov/client/net")
var netDisconnected=_glovClientNet2.netDisconnected
var netSubs=_glovClientNet2.netSubs
var _glovClientParticles=require("../glov/client/particles")
var particles=_glovClientParticles
var _glovClientSocial=require("../glov/client/social")
var socialInit=_glovClientSocial.socialInit
...
var _glovClientUi2=require("../glov/client/ui")
var uiHandlingNav=_glovClientUi2.uiHandlingNav
...
var v2length=_glovCommonVmath.v2length
var v2scale=_glovCommonVmath.v2scale
var v2set=_glovCommonVmath.v2set
var v4copy=_glovCommonVmath.v4copy
var v4lerp=_glovCommonVmath.v4lerp
var vec2=_glovCommonVmath.vec2
var vec4=_glovCommonVmath.vec4
var _commonEntity_test_common=require("../common/entity_test_common")
var EntityType=_commonEntity_test_common.EntityType
var VA_SIZE=_commonEntity_test_common.VA_SIZE
var VIEW_DIST=_commonEntity_test_common.VIEW_DIST
var entityTestCommonClass=_commonEntity_test_common.entityTestCommonClass
var _account_ui=require("./account_ui")
var createAccountUI=_account_ui.createAccountUI
var _particle_data=require("./particle_data")
var particle_data=_particle_data
var PI=Math.PI,cos=Math.cos,max=Math.max,random=Math.random,sin=Math.sin
Z.BACKGROUND=1
Z.SPRITES=10
var AI_CLAIM_TIME=2e3
var EntityTestClient=function(_entityTestCommonClas){_inheritsLoose(EntityTestClient,_entityTestCommonClas)
function EntityTestClient(ent_id,entity_manager){var _this;(_this=_entityTestCommonClas.call(this,ent_id,entity_manager)||this).next_move_time=void 0
_this.in_view=void 0
_this.my_activate_time=void 0
_this.error_time=void 0
_this.home_point=void 0
_this.waiting=void 0
_this.waiting=false
_this.in_view=false
_this.my_activate_time=-Infinity
_this.error_time=-Infinity
_this.resetMoveTime(true)
return _this}var _proto=EntityTestClient.prototype
_proto.resetMoveTime=function resetMoveTime(initial){this.next_move_time=engine.frame_timestamp+500+random()*(500+(initial?AI_CLAIM_TIME:0))}
_proto.applyAIMove=function applyAIMove(new_pos){var _this2=this
assert(!this.waiting)
this.waiting=true
this.applyBatchUpdate({field:EntityTestClient.AI_MOVE_FIELD,action_id:"ai_move",data_assignments:{pos:new_pos}},function(err){if(err)_this2.error_time=engine.frame_timestamp
_this2.waiting=false})}
_proto.erroredRecently=function erroredRecently(){var dt=engine.frame_timestamp-this.error_time
return max(0,1-dt/2e3)}
_proto.activatedRecently=function activatedRecently(){var dt=engine.frame_timestamp-this.my_activate_time
return max(0,1-dt/2e3)}
_proto.lastUpdatedBySomeoneElse=function lastUpdatedBySomeoneElse(){if(!this.data.seq_ai_move)return false
if(this.data.seq_ai_move.startsWith(this.entity_manager.sub_id_prefix))return false
return true}
_proto.tickAI=function tickAI(dt){if(this.waiting||!this.in_view)return
if(engine.frame_timestamp<this.next_move_time)return
if(this.lastUpdatedBySomeoneElse())if(engine.frame_timestamp-this.last_update_timestamp<AI_CLAIM_TIME){this.resetMoveTime(false)
return}var pos=this.getData("pos")
assert(pos)
if(!this.home_point)this.home_point=pos.slice(0)
var r=64*random()
var theta=random()*PI*2
var new_pos=[this.home_point[0]+sin(theta)*r,this.home_point[1]-cos(theta)*r,theta]
this.my_activate_time=engine.frame_timestamp
this.resetMoveTime(false)
this.applyAIMove(new_pos)}
return EntityTestClient}(entityTestCommonClass(EntityBaseClient))
EntityTestClient.AI_MOVE_FIELD="seq_ai_move"
var entity_manager
var entity_pos_manager
var ROOM_REQUIRES_LOGIN=false
var game_width=1280
var game_height=960
var SPEED=.2
var sprite_entity
var sprite_game_bg
var account_ui=createAccountUI()
var chat_ui
var test_character={pos:vec2(),rot:0}
function onEntReady(){if(entity_manager.hasMyEnt()){var pos=entity_manager.getMyEnt().getData("pos",[0,0])
v2copy(test_character.pos,pos)}}var color_self=vec4(.5,1,.5,1)
var color_self_shadow=vec4(0,1,0,.25)
var color_player=vec4(.75,.5,1,1)
var color_bot=vec4(.25,.25,.25,1)
var color_bot_active=vec4(1,.66,.25,1)
var color_bot_error=vec4(1,0,0,1)
function main(){net.init({engine:engine,cmd_parse:cmd_parse,auto_create_user:false})
if(!engine.startup({game_width:game_width,game_height:game_height,pixely:false,font:{info:require("./img/font/palanquin32.json"),texture:"font/palanquin32"},safearea_ignore_bottom:true,ui_sounds:{msg_err:"msg_err",msg_in:"msg_in",msg_out:"msg_out",msg_out_err:"msg_out_err",user_join:"user_join",user_leave:"user_leave"}}))return
chat_ui=chatUICreate({max_len:1e3})
entity_manager=clientEntityManagerCreate({channel_type:"enttest",EntityCtor:EntityTestClient})
entity_pos_manager=entityPositionManagerCreate({entity_manager:entity_manager,dim_pos:2,dim_rot:1,anim_state_defs:{test_anim_state:{stopped:function stopped(value){return"idle"===value}}}})
entity_manager.on("ent_ready",onEntReady)
netSubs().onChannelMsg("enttest","initial_pos",function(data){v2copy(test_character.pos,data)})
socialInit()
var KEYS=input.KEYS
var PAD=input.PAD
var color_temp=vec4()
...
sprite_entity=spriteCreate({name:"entity",ws:[128,128],hs:[128,128],size:vec2(sprite_size,sprite_size),origin:vec2(.5,.5)})
sprite_game_bg=spriteCreate({url:"white",size:vec2(game_width,game_height)})}var test_room=null
var impulse=vec2()
var player_state={test_anim_state:"idle"}
function playerMotion(dt){if(entity_manager.checkNet())return
if(uiHandlingNav())return
v2set(impulse,0,0)
...
player_state.test_anim_state=v2length(impulse)>.1*dt?"walk":"idle"
v2addScale(test_character.pos,test_character.pos,impulse,SPEED)
entity_pos_manager.updateMyPos(new Float64Array([test_character.pos[0],test_character.pos[1],test_character.rot]),player_state)}var last_ai_tick=engine.frame_timestamp
function aiMotion(dt){if(engine.frame_timestamp<last_ai_tick+200)return
last_ai_tick=engine.frame_timestamp
var entities=entity_manager.entities
for(var ent_id_string in entities){var ent=entities[Number(ent_id_string)]
if(ent.data.type!==EntityType.Bot)continue
ent.tickAI(dt)}}function getRoom(){if(!test_room&&!netDisconnected()){test_room=netSubs().getChannel("enttest.test",true)
assert(test_room)
entity_manager.reinit({channel:test_room})
entity_pos_manager.reinit()
chat_ui.setChannel(test_room)}}function preLogout(){if(test_room){assert(test_room.numSubscriptions())
test_room.unsubscribe()
chat_ui.setChannel(null)
test_room=null
if(!ROOM_REQUIRES_LOGIN)setTimeout(getRoom,1)}}var pad_controls_sprite=true
var was_active=false
function test(dt){if(pad_controls_sprite)spotSuppressPad()
entity_manager.tick()
chat_ui.run()
var y=0
y=account_ui.showLogin({x:0,y:y,prelogout:preLogout,prelogin:preLogout,center:false,style:glov_font.style(null,{outline_width:2,outline_color:4294967295,color:255})})+4
if(test_room&&test_room.numSubscriptions()&&entity_manager.isReady()&&!netDisconnected()){if(!was_active)was_active=pad_controls_sprite=true
playerMotion(dt)
aiMotion(dt)
if(ui.buttonText({x:0,y:y,text:"Spawn Entity"})){var r=128
var theta=random()*PI*2
var pos=[test_character.pos[0]+sin(theta)*r,test_character.pos[1]+cos(theta)*r,0]
test_room.send("spawn",{pos:pos})}y+=ui.button_height+4
if(ui.buttonText({x:0,y:y,text:"Reset Current VA"}))test_room.send("resetva")
y+=ui.button_height+4
sprite_game_bg.draw({x:0,y:0,z:Z.BACKGROUND,color:[.3,.32,.35,1]})
sprite_entity.draw({x:test_character.pos[0],y:test_character.pos[1],z:Z.SPRITES,rot:test_character.rot,color:color_self,frame:"idle"===player_state.test_anim_state?1:3})
ui.drawCircle(test_character.pos[0],test_character.pos[1],Z.SPRITES-2,VIEW_DIST,.99,[1,1,1,.5])
for(var xx=VA_SIZE;xx<game_width;xx+=VA_SIZE)ui.drawLine(xx,0,xx,game_height,Z.SPRITES-3,1,1,[0,0,1,.5])
for(var yy=VA_SIZE;yy<game_height;yy+=VA_SIZE)ui.drawLine(0,yy,game_width,yy,Z.SPRITES-3,1,1,[0,0,1,.5])
for(var _xx=VA_SIZE/2;_xx<game_width;_xx+=VA_SIZE)ui.drawLine(_xx,0,_xx,game_height,Z.SPRITES-3,1,1,[0,0,1,.125])
for(var _yy=VA_SIZE/2;_yy<game_height;_yy+=VA_SIZE)ui.drawLine(0,_yy,game_width,_yy,Z.SPRITES-3,1,1,[0,0,1,.125])
var entities=entity_manager.entities
for(var key in entities){var ent_id=Number(key)
var ent=entities[ent_id]
var ped=entity_pos_manager.updateOtherEntity(ent_id,dt)
if(!ped)continue
var _pos=ped.pos
var frame=void 0
if(ent.data.type===EntityType.Player){if(ent_id===entity_manager.my_ent_id)v4copy(color_temp,color_self_shadow)
else v4copy(color_temp,color_player)
frame="idle"===ped.anim_state.test_anim_state?1:3}else{v4lerp(color_temp,ent.activatedRecently(),color_bot,color_bot_active)
v4lerp(color_temp,ent.erroredRecently(),color_temp,color_bot_error)
frame=0}color_temp[3]*=null!==ent.fade?ent.fade:1
if(v2dist(test_character.pos,_pos)>VIEW_DIST){color_temp[3]*=.25
ent.in_view=false}else ent.in_view=true
sprite_entity.draw({x:_pos[0],y:_pos[1],z:Z.SPRITES-1,rot:_pos[2],color:color_temp,frame:frame})
if(ent.data.display_name)ui.font.drawSizedAligned(glov_font.styleAlpha(glov_font.styleColored(null,128),color_temp[3]),_pos[0],_pos[1]-64,Z.SPRITES-1,ui.font_height,glov_font.ALIGN.HCENTER,0,0,ent.data.display_name)}}else was_active=pad_controls_sprite=false
chat_ui.runLate()
if(input.keyDownEdge(KEYS.ESC)||input.padButtonDownEdge(PAD.B))pad_controls_sprite=!pad_controls_sprite
entity_manager.actionListFlush()}function testInit(dt){engine.setState(test)
if(!ROOM_REQUIRES_LOGIN)netSubs().onceConnected(getRoom)
netSubs().onLogin(getRoom)
test(dt)}initGraphics()
engine.setState(testInit)}

},{"../common/entity_test_common":11,"../glov/client/chat_ui":16,"../glov/client/cmds":18,"../glov/client/engine":23,"../glov/client/entity_base_client":24,"../glov/client/entity_manager_client":25,"../glov/client/entity_position_manager":26,"../glov/client/font":32,"../glov/client/input":42,"../glov/client/local_storage":45,"../glov/client/net":50,"../glov/client/particles":52,"../glov/client/social":68,"../glov/client/spot":71,"../glov/client/sprites":74,"../glov/client/ui":78,"../glov/common/vmath":101,"./account_ui":1,"./img/font/palanquin32.json":6,"./particle_data":9,"assert":undefined}],4:[function(require,module,exports){
module.exports={"font_size":8,"imageW":128,"imageH":128,"spread":2,"noFilter":1,"channels":1,"char_infos":[{"c":2,"x0":2,"y0":2,"xpad":1,"w":5,"h":8},{"c":32,"xpad":4},{"c":33,"x0":102,"y0":2,"yoffs":1,"xpad":1,"w":1,"h":5},{"c":34,"x0":61,"y0":63,"yoffs":1,"xpad":1,"w":3,"h":2},{"c":35,"x0":108,"y0":2,"yoffs":1,"xpad":1,"w":5,"h":5},{"c":36,"x0":48,"y0":2,"yoffs":1,"xpad":1,"w":4,"h":6},{"c":37,"x0":118,"y0":2,"yoffs":1,"xpad":1,"w":5,"h":5},{"c":38,"x0":2,"y0":15,"yoffs":1,"xpad":1,"w":5,"h":5},{"c":39,"x0":69,"y0":62,"yoffs":1,"xpad":1,"w":1,"h":2},{"c":40,"x0":12,"y0":14,"yoffs":1,"xpad":1,"w":2,"h":5},{"c":41,"x0":19,"y0":14,"yoffs":1,"xpad":1,"w":2,"h":5},{"c":42,"x0":21,"y0":64,"yoffs":1,"xpad":1,"w":3,"h":3},{"c":43,"x0":29,"y0":63,"yoffs":2,"xpad":1,"w":3,"h":3},{"c":44,"x0":75,"y0":62,"yoffs":5,"xpad":1,"w":2,"h":2},{"c":45,"x0":106,"y0":62,"yoffs":3,"xpad":1,"w":3,"h":1},{"c":46,"x0":114,"y0":62,"yoffs":5,"xpad":1,"w":1,"h":1},{"c":47,"x0":26,"y0":14,"yoffs":1,"xpad":1,"w":5,"h":5},{"c":48,"x0":36,"y0":14,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":49,"x0":45,"y0":14,"yoffs":1,"xpad":1,"w":2,"h":5},{"c":50,"x0":52,"y0":13,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":51,"x0":61,"y0":13,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":52,"x0":70,"y0":13,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":53,"x0":79,"y0":13,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":54,"x0":88,"y0":13,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":55,"x0":97,"y0":13,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":56,"x0":106,"y0":12,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":57,"x0":115,"y0":12,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":58,"x0":37,"y0":63,"yoffs":2,"xpad":1,"w":1,"h":3},{"c":59,"x0":12,"y0":55,"yoffs":2,"xpad":1,"w":1,"h":4},{"c":60,"x0":2,"y0":25,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":61,"x0":43,"y0":63,"yoffs":2,"xpad":1,"w":3,"h":3},{"c":62,"x0":10,"y0":25,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":63,"x0":18,"y0":24,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":64,"x0":27,"y0":24,"yoffs":1,"xpad":1,"w":5,"h":5},{"c":65,"x0":37,"y0":24,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":66,"x0":46,"y0":24,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":67,"x0":55,"y0":23,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":68,"x0":63,"y0":23,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":69,"x0":72,"y0":23,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":70,"x0":80,"y0":23,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":71,"x0":88,"y0":23,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":72,"x0":97,"y0":23,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":73,"x0":106,"y0":22,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":74,"x0":114,"y0":22,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":75,"x0":2,"y0":35,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":76,"x0":11,"y0":35,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":77,"x0":19,"y0":34,"yoffs":1,"xpad":1,"w":5,"h":5},{"c":78,"x0":29,"y0":34,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":79,"x0":36,"y0":14,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":80,"x0":38,"y0":34,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":81,"x0":57,"y0":2,"yoffs":1,"xpad":1,"w":4,"h":6},{"c":82,"x0":47,"y0":34,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":83,"x0":56,"y0":33,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":84,"x0":65,"y0":33,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":85,"x0":73,"y0":33,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":86,"x0":82,"y0":33,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":87,"x0":91,"y0":33,"yoffs":1,"xpad":1,"w":5,"h":5},{"c":88,"x0":101,"y0":33,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":89,"x0":110,"y0":32,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":90,"x0":119,"y0":32,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":91,"x0":2,"y0":45,"yoffs":1,"xpad":1,"w":2,"h":5},{"c":92,"x0":9,"y0":45,"yoffs":1,"xpad":1,"w":5,"h":5},{"c":93,"x0":19,"y0":44,"yoffs":1,"xpad":1,"w":2,"h":5},{"c":94,"x0":82,"y0":62,"yoffs":1,"xpad":1,"w":3,"h":2},{"c":95,"x0":120,"y0":61,"yoffs":5,"xpad":1,"w":4,"h":1},{"c":96,"x0":90,"y0":62,"yoffs":1,"xpad":1,"w":2,"h":2},{"c":97,"x0":18,"y0":55,"yoffs":2,"xpad":1,"w":4,"h":4},{"c":98,"x0":26,"y0":44,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":99,"x0":27,"y0":54,"yoffs":2,"xpad":1,"w":3,"h":4},{"c":100,"x0":35,"y0":44,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":101,"x0":35,"y0":54,"yoffs":2,"xpad":1,"w":4,"h":4},{"c":102,"x0":44,"y0":44,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":103,"x0":66,"y0":2,"yoffs":2,"xpad":1,"w":4,"h":6},{"c":104,"x0":52,"y0":44,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":105,"x0":61,"y0":43,"yoffs":1,"xpad":1,"w":1,"h":5},{"c":106,"x0":12,"y0":2,"yoffs":1,"xpad":1,"w":2,"h":7},{"c":107,"x0":67,"y0":43,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":108,"x0":76,"y0":43,"yoffs":1,"xpad":1,"w":1,"h":5},{"c":109,"x0":44,"y0":54,"yoffs":2,"xpad":1,"w":5,"h":4},{"c":110,"x0":54,"y0":54,"yoffs":2,"xpad":1,"w":4,"h":4},{"c":111,"x0":63,"y0":53,"yoffs":2,"xpad":1,"w":4,"h":4},{"c":112,"x0":75,"y0":2,"yoffs":2,"xpad":1,"w":4,"h":6},{"c":113,"x0":84,"y0":2,"yoffs":2,"xpad":1,"w":4,"h":6},{"c":114,"x0":72,"y0":53,"yoffs":2,"xpad":1,"w":3,"h":4},{"c":115,"x0":80,"y0":53,"yoffs":2,"xpad":1,"w":4,"h":4},{"c":116,"x0":82,"y0":43,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":117,"x0":89,"y0":53,"yoffs":2,"xpad":1,"w":4,"h":4},{"c":118,"x0":98,"y0":53,"yoffs":2,"xpad":1,"w":4,"h":4},{"c":119,"x0":107,"y0":53,"yoffs":2,"xpad":1,"w":5,"h":4},{"c":120,"x0":117,"y0":52,"yoffs":2,"xpad":1,"w":3,"h":4},{"c":121,"x0":93,"y0":2,"yoffs":2,"xpad":1,"w":4,"h":6},{"c":122,"x0":2,"y0":65,"yoffs":2,"xpad":1,"w":4,"h":4},{"c":123,"x0":90,"y0":43,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":124,"x0":76,"y0":43,"yoffs":1,"xpad":1,"w":1,"h":5},{"c":125,"x0":98,"y0":43,"yoffs":1,"xpad":1,"w":3,"h":5},{"c":126,"x0":97,"y0":62,"yoffs":1,"xpad":1,"w":4,"h":2},{"c":160,"xpad":4},{"c":181,"x0":106,"y0":43,"yoffs":2,"xpad":1,"w":4,"h":5},{"c":916,"x0":11,"y0":65,"yoffs":2,"xpad":1,"w":5,"h":4},{"c":9654,"x0":115,"y0":42,"yoffs":1,"xpad":1,"w":4,"h":5},{"c":9660,"x0":51,"y0":63,"yoffs":2,"xpad":1,"w":5,"h":3},{"c":9742,"x0":19,"y0":2,"yoffs":1,"xpad":1,"w":10,"h":7},{"c":9743,"x0":34,"y0":2,"yoffs":1,"xpad":1,"w":9,"h":7},{"c":65533,"x0":2,"y0":55,"yoffs":1,"xpad":1,"w":5,"h":5}]}
},{}],5:[function(require,module,exports){
module.exports={"font_size":16,"imageW":1024,"imageH":64,"spread":2,"noFilter":1,"channels":1,"char_infos":[{"c":2,"x0":2,"y0":2,"xpad":1,"w":10,"h":16},{"c":32,"x0":215,"y0":17,"xpad":7,"w":1,"h":1},{"c":33,"x0":152,"y0":2,"yoffs":3,"xpad":2,"w":2,"h":10},{"c":34,"x0":124,"y0":19,"yoffs":3,"xpad":2,"w":6,"h":4},{"c":35,"x0":159,"y0":2,"yoffs":3,"xpad":2,"w":10,"h":10},{"c":36,"x0":74,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":12},{"c":37,"x0":174,"y0":2,"yoffs":3,"xpad":2,"w":10,"h":10},{"c":38,"x0":189,"y0":2,"yoffs":3,"xpad":2,"w":10,"h":10},{"c":39,"x0":135,"y0":19,"yoffs":3,"xpad":2,"w":2,"h":4},{"c":40,"x0":204,"y0":2,"yoffs":3,"xpad":2,"w":4,"h":10},{"c":41,"x0":213,"y0":2,"yoffs":3,"xpad":2,"w":4,"h":10},{"c":42,"x0":69,"y0":21,"yoffs":3,"xpad":2,"w":6,"h":6},{"c":43,"x0":80,"y0":19,"yoffs":5,"xpad":2,"w":6,"h":6},{"c":44,"x0":142,"y0":19,"yoffs":11,"xpad":2,"w":4,"h":4},{"c":45,"x0":184,"y0":17,"yoffs":7,"xpad":2,"w":6,"h":2},{"c":46,"x0":195,"y0":17,"yoffs":11,"xpad":2,"w":2,"h":2},{"c":47,"x0":222,"y0":2,"yoffs":3,"xpad":2,"w":10,"h":10},{"c":48,"x0":237,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":49,"x0":250,"y0":2,"yoffs":3,"xpad":2,"w":4,"h":10},{"c":50,"x0":259,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":51,"x0":272,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":52,"x0":285,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":53,"x0":298,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":54,"x0":311,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":55,"x0":324,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":56,"x0":337,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":57,"x0":350,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":58,"x0":91,"y0":19,"yoffs":5,"xpad":2,"w":2,"h":6},{"c":59,"x0":898,"y0":2,"yoffs":5,"xpad":2,"w":2,"h":8},{"c":60,"x0":363,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":61,"x0":98,"y0":19,"yoffs":5,"xpad":2,"w":6,"h":6},{"c":62,"x0":374,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":63,"x0":385,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":64,"x0":398,"y0":2,"yoffs":3,"xpad":2,"w":10,"h":10},{"c":65,"x0":413,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":66,"x0":426,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":67,"x0":439,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":68,"x0":450,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":69,"x0":463,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":70,"x0":474,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":71,"x0":485,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":72,"x0":498,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":73,"x0":511,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":74,"x0":522,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":75,"x0":535,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":76,"x0":548,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":77,"x0":559,"y0":2,"yoffs":3,"xpad":2,"w":10,"h":10},{"c":78,"x0":574,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":79,"x0":237,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":80,"x0":587,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":81,"x0":87,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":12},{"c":82,"x0":600,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":83,"x0":613,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":84,"x0":626,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":85,"x0":637,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":86,"x0":650,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":87,"x0":663,"y0":2,"yoffs":3,"xpad":2,"w":10,"h":10},{"c":88,"x0":678,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":89,"x0":691,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":90,"x0":704,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":91,"x0":715,"y0":2,"yoffs":3,"xpad":2,"w":4,"h":10},{"c":92,"x0":724,"y0":2,"yoffs":3,"xpad":2,"w":10,"h":10},{"c":93,"x0":739,"y0":2,"yoffs":3,"xpad":2,"w":4,"h":10},{"c":94,"x0":151,"y0":19,"yoffs":3,"xpad":2,"w":6,"h":4},{"c":95,"x0":202,"y0":17,"yoffs":11,"xpad":2,"w":8,"h":2},{"c":96,"x0":162,"y0":17,"yoffs":3,"xpad":2,"w":4,"h":4},{"c":97,"x0":905,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":8},{"c":98,"x0":748,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":99,"x0":918,"y0":2,"yoffs":5,"xpad":2,"w":6,"h":8},{"c":100,"x0":761,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":101,"x0":929,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":8},{"c":102,"x0":774,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":103,"x0":100,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":12},{"c":104,"x0":785,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":105,"x0":798,"y0":2,"yoffs":3,"xpad":2,"w":2,"h":10},{"c":106,"x0":17,"y0":2,"yoffs":3,"xpad":2,"w":4,"h":14},{"c":107,"x0":805,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":108,"x0":818,"y0":2,"yoffs":3,"xpad":2,"w":2,"h":10},{"c":109,"x0":942,"y0":2,"yoffs":5,"xpad":2,"w":10,"h":8},{"c":110,"x0":957,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":8},{"c":111,"x0":970,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":8},{"c":112,"x0":113,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":12},{"c":113,"x0":126,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":12},{"c":114,"x0":983,"y0":2,"yoffs":5,"xpad":2,"w":6,"h":8},{"c":115,"x0":994,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":8},{"c":116,"x0":825,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":117,"x0":1007,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":8},{"c":118,"x0":2,"y0":23,"yoffs":5,"xpad":2,"w":8,"h":8},{"c":119,"x0":15,"y0":23,"yoffs":5,"xpad":2,"w":10,"h":8},{"c":120,"x0":30,"y0":21,"yoffs":5,"xpad":2,"w":6,"h":8},{"c":121,"x0":139,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":12},{"c":122,"x0":41,"y0":21,"yoffs":5,"xpad":2,"w":8,"h":8},{"c":123,"x0":836,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":124,"x0":818,"y0":2,"yoffs":3,"xpad":2,"w":2,"h":10},{"c":125,"x0":847,"y0":2,"yoffs":3,"xpad":2,"w":6,"h":10},{"c":126,"x0":171,"y0":17,"yoffs":3,"xpad":2,"w":8,"h":4},{"c":160,"x0":215,"y0":17,"xpad":7,"w":1,"h":1},{"c":181,"x0":858,"y0":2,"yoffs":5,"xpad":2,"w":8,"h":10},{"c":916,"x0":54,"y0":21,"yoffs":5,"xpad":2,"w":10,"h":8},{"c":9654,"x0":871,"y0":2,"yoffs":3,"xpad":2,"w":8,"h":10},{"c":9660,"x0":109,"y0":19,"yoffs":5,"xpad":2,"w":10,"h":6},{"c":9742,"x0":26,"y0":2,"yoffs":3,"xpad":2,"w":20,"h":14},{"c":9743,"x0":51,"y0":2,"yoffs":3,"xpad":2,"w":18,"h":14},{"c":65533,"x0":884,"y0":2,"yoffs":3,"xpad":1,"w":9,"h":10}]}
...
var _glovClientSpotJs=require("../glov/client/spot.js")
var spotSuppressPad=_glovClientSpotJs.spotSuppressPad
var _glovClientSprite_animationJs=require("../glov/client/sprite_animation.js")
var spriteAnimationCreate=_glovClientSprite_animationJs.spriteAnimationCreate
...
var spriteCreate=_glovClientSpritesJs2.spriteCreate
...
var flags={}
function flagGet(key,dflt){if(void 0===flags[key])flags[key]=local_storage.getJSON("flag_"+key,dflt)||false
return flags[key]}function flagToggle(key){flags[key]=!flagGet(key)
...
function perfTestSprites(){if(!sprites.test)sprites.test=[spriteCreate({name:"test",size:vec2(1,1),origin:vec2(.5,.5)}),spriteCreate({url:"img/test.png?1",size:vec2(1,1),origin:vec2(.5,.5)})]
var mode=4
var count=[8e4,4e4,2e4,6e4,6e4][mode]
if(3===mode||4===mode){var z=0
for(var ii=0;ii<count;){var subc=floor(500+100*Math.random())
var idx=mode<=1?0:Math.round(Math.random())
var sprite=sprites.test[idx]
for(var jj=0;jj<subc;++jj){if(4===mode)glov_sprites.queuesprite(sprite,Math.random()*game_width,Math.random()*game_height,z,6*sprite.size[0],6*sprite.size[1],0,sprite.uvs,color_white)
else sprites.test[idx].draw({x:Math.random()*game_width,y:Math.random()*game_height,z:z,w:6,h:6})
z+=.01}ii+=subc}}else for(var _ii=0;_ii<count;++_ii){var _idx=mode<=1?0:Math.round(Math.random())
sprites.test[_idx].draw({x:Math.random()*game_width,y:Math.random()*game_height,z:0===mode?_ii:Math.random(),w:6,h:6})}}var color_black=vec4(0,0,0,1)
var line_precise=1
function lineTest(){var line_len=20
var y_values=[20,25.25,30.5,35.667,45+5*sin(.001*engine.frame_timestamp)]
...
for(var jj=0;jj<y_values.length;++jj){var x=x0+2*jj
var y=y_values[jj]
...
z+=.1}}ui.drawLine(50,72,250,200,z,20,line_precise,color_black,ui.LINE_CAP_ROUND)}function main(){if(engine.DEBUG)net.init({engine:engine})
...
var pixely=flagGet("pixely","on")
var font
var ui_sprites
if("strict"===pixely){font={info:font_info_04b03x1,texture:"font/04b03_8x1"}
ui_sprites=spriteSetGet("pixely")}else if(pixely&&"off"!==pixely){font={info:font_info_04b03x2,texture:"font/04b03_8x2"}
ui_sprites=spriteSetGet("pixely")}else font={info:font_info_palanquin32,texture:"font/palanquin32"}
if(!engine.startup({game_width:game_width,game_height:game_height,pixely:pixely,font:font,viewport_postprocess:false,ui_sprites:ui_sprites}))return
font=engine.font
ui.scaleSizes(13/32)
ui.setFontHeight(8)
...
sprites.test_tint=spriteCreate({name:"tinted",ws:[16,16,16,16],hs:[16,16,16],size:vec2(sprite_size,sprite_size),layers:2,origin:vec2(.5,.5)})
sprites.animation=spriteAnimationCreate({idle_left:{frames:[0,1],times:[200,500]},idle_right:{frames:[3,2],times:[200,500]}})
sprites.animation.setState("idle_left")
sprites.game_bg=spriteCreate({url:"white",size:vec2(game_width,game_height)})}var spine_inited=false
var spine_anim
...
var pad_controls_sprite=false
function test(dt){gl.clearColor(0,.72,1,1)
if(!test.color_sprite){test.color_sprite=v4clone(color_white)
test.character={x:Math.random()*(game_width-sprite_size-200)+.5*sprite_size+200,y:Math.random()*(game_height-sprite_size)+.5*sprite_size}}if(pad_controls_sprite)spotSuppressPad()
if(flagGet("ui_test"))ui_test.run(10,10,Z.UI_TEST)
if(flagGet("font_test"))ui_test.runFontTest(105,20)
if(flagGet("lines"))lineTest()
test.character.dx=0
test.character.dy=0
...
if(spine_anim)spine_anim.setAnimation(0,"run",true)}else if(spine_anim)if("run"===spine_anim.getAnimation(0))spine_anim.setAnimation(0,"idle",true)
else if("run_left"===spine_anim.getAnimation(0))spine_anim.setAnimation(0,"idle_left",true)
test.character.x+=.05*test.character.dx
test.character.y+=.05*test.character.dy
var bounds={x:test.character.x-sprite_size/2,y:test.character.y-sprite_size/2,w:sprite_size,h:sprite_size}
if(input.mouseDownOverBounds(bounds))v4copy(test.color_sprite,color_yellow)
else if(input.click(bounds)){v4copy(test.color_sprite,0===test.color_sprite[2]?color_white:color_red)
...
test.color_sprite[3]=1}if(flagGet("spine")){if(!spine_inited)spineInit()
spine_anim.update(2*dt)
spine_anim.draw({x:test.character.x,y:test.character.y,z:Z.SPRITES,scale:.25})}else sprites.test_tint.drawDualTint({x:test.character.x,y:test.character.y,z:Z.SPRITES,color:[1,1,0,1],color1:[1,0,1,1],frame:sprites.animation.getFrame(dt)})
if(flagGet("4color"))sprites.test_tint.draw4Color({x:test.character.x,y:test.character.y+64,z:Z.SPRITES,c0:[1,0,0,1],c1:[0,1,0,1],c2:[0,0,1,1],c3:[1,0,1,1],frame:sprites.animation.getFrame()})
var font_test_idx=0
ui.print(glov_font.styleColored(null,255),test.character.x,test.character.y+20*++font_test_idx,Z.SPRITES,"TEXT!")
...
var y=game_height-10-7*button_spacing
...
if((x+=2+mini_button_w)>=ui.button_width){x=ui.button_height
y+=button_spacing}return ret}if(ui.buttonText({x:x,y:y,text:"Pixely: "+(flagGet("pixely")||"Off"),tooltip:"Toggles pixely or regular mode (requires reload)"})){if("strict"===flagGet("pixely"))flagSet("pixely",false)
else if("on"===flagGet("pixely"))flagSet("pixely","strict")
else flagSet("pixely","on")
if(document.location.reload)document.location.reload()
else document.location=String(document.location)}y+=button_spacing
var rs3d_disabled=!flagGet("3d_test")||engine.render_width
if(ui.buttonText({x:x,y:y,text:"RenderScale3D: "+(rs3d_disabled?"":settings.render_scale),tooltip:"Changes render_scale",disabled:rs3d_disabled}))if(1===settings.render_scale)settings.set("render_scale",.25)
else settings.set("render_scale",1)
y+=button_spacing
if(ui.buttonText({x:x,y:y,text:"RenderScaleAll: "+settings.render_scale_all,tooltip:"Changes render_scale_all"}))if(1===settings.render_scale_all)settings.set("render_scale_all",.5)
else settings.set("render_scale_all",1)
y+=button_spacing
...
if(flagGet("lines"))flagSet("ui_test",false)}if(miniButton("FX","Toggles particles",flagGet("particles")))flagToggle("particles")
if(miniButton("Music","Toggles playing a looping background music track",flagGet("music"))){flagToggle("music")
if(flagGet("music"))soundPlayMusic(music_file,1,FADE_IN)
else soundPlayMusic(music_file,0,FADE_OUT)}if(miniButton("Spine","Toggles Spine animation testing",flagGet("spine")))flagToggle("spine")
if(flagGet("particles"))if(engine.getFrameTimestamp()-last_particles>1e3){last_particles=engine.getFrameTimestamp()
engine.glov_particles.createSystem(particle_data.defs.explosion,[100+120*Math.random(),100+140*Math.random(),Z.PARTICLES])}if(flagGet("perf_test"))perfTestSprites()
if(do_3d)test3D()
if(input.keyDownEdge(KEYS.ESC)||input.padButtonDownEdge(PAD.B))pad_controls_sprite=!pad_controls_sprite}function testInit(dt){engine.setState(test)
if(flagGet("music"))soundPlayMusic(music_file)
test(dt)}initGraphics()
engine.setState(testInit)}

},{"../glov/client/engine.js":23,"../glov/client/font.js":32,"../glov/client/input.js":42,"../glov/client/local_storage.js":45,"../glov/client/net.js":50,"../glov/client/particles.js":52,"../glov/client/settings.js":63,"../glov/client/slider.js":67,"../glov/client/sound.js":69,"../glov/client/spine.js":70,"../glov/client/spot.js":71,"../glov/client/sprite_animation.js":72,"../glov/client/sprite_sets.js":73,"../glov/client/sprites.js":74,"../glov/client/transition.js":77,"../glov/client/ui.js":78,"../glov/client/ui_test.js":79,"../glov/common/vmath.js":101,"./img/font/04b03_8x1.json":4,"./img/font/04b03_8x2.json":5,"./img/font/palanquin32.json":6,"./particle_data.js":9,"./test_3d.js":10}],8:[function(require,module,exports){
"use strict"
exports.main=main
...
var _glovClientNet2=require("../glov/client/net")
var netSubs=_glovClientNet2.netSubs
var _glovClientNet_position_manager=require("../glov/client/net_position_manager")
var net_position_manager=_glovClientNet_position_manager
var _glovClientParticles=require("../glov/client/particles")
var particles=_glovClientParticles
var _glovClientShaders=require("../glov/client/shaders")
var shaders=_glovClientShaders
var _glovClientSocial=require("../glov/client/social")
var socialInit=_glovClientSocial.socialInit
...
var _glovClientSpot=require("../glov/client/spot")
var spotSuppressPad=_glovClientSpot.spotSuppressPad
var _glovClientSprite_animation=require("../glov/client/sprite_animation")
var spriteAnimationCreate=_glovClientSprite_animation.spriteAnimationCreate
var _glovClientSprites=require("../glov/client/sprites")
var spriteCreate=_glovClientSprites.spriteCreate
var _glovClientUi=require("../glov/client/ui")
var ui=_glovClientUi
...
var toNumber=_glovCommonUtil.toNumber
var _glovCommonVmath=require("../glov/common/vmath")
var v2sub=_glovCommonVmath.v2sub
var vec2=_glovCommonVmath.vec2
...
var account_ui
var chat_ui
cmd_parse.register({cmd:"bin_get",func:function func(str,resp_func){chat_ui.channel.pak("bin_get").send(function(err,pak){if(err)return void resp_func(err)
resp_func(null,pak.readBuffer(false).join(","))})}})
cmd_parse.register({cmd:"bin_set",func:function func(str,resp_func){var pak=chat_ui.channel.pak("bin_set")
...
soundLoad("test")
sprites.white=spriteCreate({url:"white"})
sprites.test=spriteCreate({name:"test",size:vec2(sprite_size,sprite_size),origin:vec2(.5,.5)})
sprites.test_tint=spriteCreate({name:"tinted",ws:[16,16,16,16],hs:[16,16,16],size:vec2(sprite_size,sprite_size),layers:2,origin:vec2(.5,.5)});(animation=spriteAnimationCreate({idle_left:{frames:[0,1],times:[200,500]},idle_right:{frames:[3,2],times:[200,500]}})).setState("idle_left")
sprites.game_bg=spriteCreate({url:"white",size:vec2(game_width,game_height)})}var test_room=null
var test_character={x:0,y:0,rot:0}
function playerMotion(dt){if(pos_manager.checkNet(function(pos){test_character.x=pos[0]
test_character.y=pos[1]
test_character.rot=pos[2]}))return
if(uiHandlingNav())return
var dx=0
...
if(dx<0)animation.setState("idle_left")
else if(dx>0)animation.setState("idle_right")
test_character.x+=.2*dx
test_character.y+=.2*dy
var aim=v2sub(vec2(),input.mousePos(),[test_character.x,test_character.y])
test_character.rot=atan2(aim[0],-aim[1])
pos_manager.updateMyPos(new Float64Array([test_character.x,test_character.y,test_character.rot]),"idle")}function getRoom(){if(!test_room){test_room=netSubs().getChannel("multiplayer.test",true)
pos_manager.reinit({channel:test_room,default_pos:vec3(random()*(game_width-sprite_size)+.5*sprite_size,random()*(game_height-sprite_size)+.5*sprite_size,0)})
chat_ui.setChannel(test_room)}}function preLogout(){if(test_room){assert(test_room.numSubscriptions())
test_room.unsubscribe()
chat_ui.setChannel(null)
test_room=null
...
function test(dt){if(pad_controls_sprite)spotSuppressPad()
chat_ui.run()
account_ui.showLogin({x:0,y:0,prelogout:preLogout,center:false,style:glov_font.style(null,{outline_width:2,outline_color:4294967295,color:255})})
if(test_room&&test_room.numSubscriptions()){if(!was_active)was_active=pad_controls_sprite=true
playerMotion(dt)
sprites.game_bg.draw({x:0,y:0,z:Z.BACKGROUND,color:[.5,.6,.7,1],shader:test_shader,shader_params:{params:[1,1,1,5e-4*engine.getFrameTimestamp()%1e3]}})
sprites.test_tint.drawDualTint({x:test_character.x,y:test_character.y,z:Z.SPRITES,rot:test_character.rot,color:[1,1,0,1],color1:[1,0,1,1],frame:animation.getFrame(dt)})
...
if(other_client.pos&&other_client.ids){var ids=other_client.ids
var pcd=pos_manager.updateOtherClient(client_id,dt)
if(pcd){var pos=pcd.pos
sprites.test.draw({x:pos[0],y:pos[1],z:Z.SPRITES-1,rot:pos[2],color:color_gray})
ui.font.drawSizedAligned(glov_font.styleColored(null,128),pos[0],pos[1]-64,Z.SPRITES-1,ui.font_height,glov_font.ALIGN.HCENTER,0,0,ids.display_name||"client_"+client_id)}}}}else was_active=pad_controls_sprite=false
chat_ui.runLate()
if(input.keyDownEdge(KEYS.ESC)||input.padButtonDownEdge(PAD.B))pad_controls_sprite=!pad_controls_sprite}function testInit(dt){engine.setState(test)
if(!ROOM_REQUIRES_LOGIN)getRoom()
netSubs().onLogin(getRoom)
test(dt)}initGraphics()
engine.setState(testInit)}

},{"../glov/client/chat_ui":16,"../glov/client/cmds":18,"../glov/client/engine":23,"../glov/client/font":32,"../glov/client/input":42,"../glov/client/local_storage":45,"../glov/client/net":50,"../glov/client/net_position_manager":51,"../glov/client/particles":52,"../glov/client/shaders":65,"../glov/client/social":68,"../glov/client/sound":69,"../glov/client/spot":71,"../glov/client/sprite_animation":72,"../glov/client/sprites":74,"../glov/client/ui":78,"../glov/common/util":99,"../glov/common/vmath":101,"./account_ui":1,"./img/font/palanquin32.json":6,"./particle_data":9,"assert":undefined}],9:[function(require,module,exports){
"use strict"
exports.defs=void 0
var defs={};(exports.defs=defs).explosion={particles:{part0:{blend:"alpha",texture:"particles/circle64",color:[1,1,1,1],color_track:[{t:0,v:[1,1,1,0]},{t:.2,v:[1,1,1,1]},{t:.4,v:[1,1,.5,.5]},{t:1,v:[1,0,0,0]}],size:[[48,8],[48,8]],size_track:[{t:0,v:[1,1]},{t:.2,v:[2,2]},{t:.4,v:[1,1]},{t:1,v:[1.5,1.5]}],accel:[0,0,0],rot:[0,360],rot_vel:[10,2],lifespan:[2500,0],kill_time_accel:5}},emitters:{part0:{particle:"part0",pos:[[-16,32],[-16,32],0],vel:[0,0,0],emit_rate:[15,0],emit_time:[0,1e3],emit_initial:10,max_parts:Infinity}},system_lifespan:2500}
...
},{"../glov/client/engine.js":23,"../glov/client/mat4ScaleRotateTranslate.js":48,"../glov/client/models.js":49,"../glov/client/quat.js":59,"../glov/client/textures.js":76,"../glov/common/vmath.js":101,"gl-mat4/lookAt":undefined}],11:[function(require,module,exports){
"use strict"
exports.VIEW_DIST=exports.VA_SIZE=exports.EntityType=void 0
exports.entityTestCommonClass=entityTestCommonClass
function _inheritsLoose(subClass,superClass){subClass.prototype=Object.create(superClass.prototype)
_setPrototypeOf(subClass.prototype.constructor=subClass,superClass)}function _setPrototypeOf(o,p){return(_setPrototypeOf=Object.setPrototypeOf?Object.setPrototypeOf.bind():function _setPrototypeOf(o,p){o.__proto__=p
return o})(o,p)}var VA_SIZE=500
exports.VA_SIZE=VA_SIZE
var VIEW_DIST=200
exports.VIEW_DIST=VIEW_DIST
var EntityType;(function(EntityType){EntityType[EntityType["Player"]=1]="Player"
EntityType[EntityType["Bot"]=2]="Bot"})((exports.EntityType=EntityType)||(exports.EntityType=EntityType={}))
function entityTestCommonClass(base){return function(_base){_inheritsLoose(EntityTestCommon,_base)
function EntityTestCommon(){var _this
for(var _len=arguments.length,args=new Array(_len),_key=0;_key<_len;_key++)args[_key]=arguments[_key];(_this=_base.call.apply(_base,[this].concat(args))||this).data.pos=[0,0,0]
return _this}return EntityTestCommon}(base)}
...
},{"./polyfill.js":56}],13:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"../common/vmath.js":101,"./camera2d.js":15,"./engine.js":23,"./font.js":32,"./net.js":50,"./scroll_area.js":61,"./ui.js":78}],15:[function(require,module,exports){
"use strict"
...
},{"./engine.js":23,"assert":undefined}],16:[function(require,module,exports){
"use strict"
exports.CHAT_FLAG_USERCHAT=exports.CHAT_FLAG_EMOTE=void 0
exports.chatUICreate=chatUICreate
...
var _require9=require("./spot.js"),SPOT_NAV_DOWN=_require9.SPOT_NAV_DOWN,SPOT_NAV_LEFT=_require9.SPOT_NAV_LEFT,SPOT_NAV_RIGHT=_require9.SPOT_NAV_RIGHT,SPOT_NAV_UP=_require9.SPOT_NAV_UP,spotUnfocus=_require9.spotUnfocus
var ui=require("./ui.js")
var _require10=require("../common/util.js"),clamp=_require10.clamp,defaults=_require10.defaults,deprecate=_require10.deprecate,matchAll=_require10.matchAll
...
else for(var ii=0;ii<this.entries.length;++ii)this.entries[ii]=local_storage.getJSON("console_e"+ii)
this.resetPos()}CmdHistory.prototype.setHist=function(idx,text){this.entries[idx]=text
local_storage.setJSON("console_e"+idx,text)}
CmdHistory.prototype.add=function(text){if(!text)return
var idx=this.entries.indexOf(text)
if(-1!==idx){var target=(this.idx-1+this.entries.length)%this.entries.length
...
if(this.entries[idx]!==text)return
this.idx=idx
local_storage.setJSON("console_idx",this.idx)
this.resetPos()}
CmdHistory.prototype.resetPos=function(){this.hist_idx=this.idx
this.edit_line=""}
CmdHistory.prototype.prev=function(cur_text){if(this.hist_idx===this.idx)this.edit_line=cur_text
var idx=(this.hist_idx-1+this.entries.length)%this.entries.length
var text=this.entries[idx]
if(idx===this.idx||!text)return this.entries[this.hist_idx]||""
this.hist_idx=idx
return text||""}
CmdHistory.prototype.next=function(cur_text){if(this.hist_idx===this.idx)return cur_text||""
var idx=(this.hist_idx+1)%this.entries.length
this.hist_idx=idx
...
if(netUserId()&&netClient().connected){var _user_public_data$per
var user_channel=netSubs().getMyUserChannel()
if(null!=(user_public_data=user_channel.data&&user_channel.data.public)&&null!=(_user_public_data$per=user_public_data.permissions)&&_user_public_data$per.sysadmin)return{sysadmin:1}}return{}}function ChatUI(params){var _custom_nav
assert.equal(typeof params,"object")
assert.equal(typeof params.max_len,"number")
this.edit_text_entry=ui.createEditBox({placeholder:"Chat",initial_focus:false,auto_unfocus:true,spatial_focus:false,max_len:params.max_len,text:"",custom_nav:((_custom_nav={})[SPOT_NAV_LEFT]=null,_custom_nav[SPOT_NAV_UP]=null,_custom_nav[SPOT_NAV_RIGHT]=null,_custom_nav[SPOT_NAV_DOWN]=null,_custom_nav)})
this.channel=null
...
this.border=params.border||void 0
...
this.styles=defaults(params.styles||{},{def:glov_font.style(null,{color:4008636159,outline_width:outline_width,outline_color:255}),error:glov_font.style(null,{color:3707764991,outline_width:outline_width,outline_color:255}),link:glov_font.style(null,{color:1346437119,outline_width:outline_width,outline_color:255}),link_hover:glov_font.style(null,{color:65535,outline_width:outline_width,outline_color:255}),system:glov_font.style(null,{color:2863311615,outline_width:outline_width,outline_color:255})})
...
this.wrap_w=wrap_w
for(var ii=this.total_lines=0;ii<this.msgs.length;++ii){var elem=this.msgs[ii]
...
if(elem.flags&CHAT_FLAG_USERCHAT)if(elem.flags&CHAT_FLAG_EMOTE)elem.msg_text=elem.display_name+" "+elem.msg
else elem.msg_text="["+elem.display_name+"] "+elem.msg
else elem.msg_text=elem.msg
elem.numlines=ui.font.numLines(this.styles[elem.style]||this.styles.def,this.wrap_w,this.indent,this.active_font_height,elem.msg_text)
this.total_lines+=elem.numlines
this.msgs.push(elem)
...
elem2.numlines=0}else if(--max_msgs<=0){elem2.hidden=true
this.total_lines-=elem2.numlines
elem2.numlines=0
...
for(var _ii=this.total_lines=0;_ii<this.msgs.length;++_ii)this.total_lines+=this.msgs[_ii].numlines}}}
function toStr(val){return"string"===typeof(val=getStringIfLocalizable(val))?val:JSON.stringify(val)}ChatUI.prototype.addChat=function(msg,style){msg=toStr(msg)
console.log(msg)
...
if(settings.profanity_filter&&data.id!==(netUserId()||netClientId()))data.msg=profanityFilter(data.msg)
this.addMsgInternal(data)}
ChatUI.prototype.onMsgJoin=function(data){if(!settings.chat_show_join_leave)return
if(data.client_id!==netClientId()){if(this.volume_join_leave)ui.playUISound("user_join",this.volume_join_leave)
this.addChatFiltered({id:data.user_id||data.client_id,display_name:data.display_name||data.client_id,flags:CHAT_FLAG_EMOTE|CHAT_FLAG_USERCHAT,msg:"joined the channel",style:"join_leave"})}}
ChatUI.prototype.onMsgLeave=function(data){if(!settings.chat_show_join_leave)return
if(this.volume_join_leave)ui.playUISound("user_leave",this.volume_join_leave)
this.addChatFiltered({id:data.user_id||data.client_id,display_name:data.display_name||data.client_id,flags:CHAT_FLAG_EMOTE|CHAT_FLAG_USERCHAT,msg:"left the channel",style:"join_leave"})}
ChatUI.prototype.registerOnMsgChatCB=function(cb){assert(!this.on_chat_cb)
this.on_chat_cb=cb}
ChatUI.prototype.onMsgChat=function(data){if(this.on_chat_cb)this.on_chat_cb(data)
var msg=data.msg,style=data.style,id=data.id,client_id=data.client_id,display_name=data.display_name,flags=data.flags,ts=data.ts,quiet=data.quiet
if(!quiet&&client_id!==netClientId())if(this.volume_in)ui.playUISound("msg_in",this.volume_in)
flags=(flags||0)|CHAT_FLAG_USERCHAT
this.addChatFiltered({id:id,display_name:display_name=display_name||id,msg:msg,style:style,flags:flags,timestamp:ts,quiet:quiet})}
...
if(input.keyDownEdge(input.KEYS.RETURN))this.focus()
if(input.keyDownEdge(input.KEYS.SLASH)||input.keyDownEdge(input.KEYS.NUMPAD_DIVIDE)){this.focus()
...
ChatUI.prototype.handleCmdParse=function(err,resp){if(err)this.addChatError(err)
else if(resp)this.addChat("[system] "+toStr(resp),"system")}
...
cmd_parse.handle(this.getAccessObj(),str,function(err,resp){if(err&&cmd_parse.was_not_found)netSubs().sendCmdParse(str,handleResult)
else handleResult(err,resp)})}
...
var eff_tooltip_pad=.5*ui.tooltip_pad
...
var text_w=w-2*eff_tooltip_pad
var tooltip_y1=param.y
var alpha=1
var vis_h=2*eff_tooltip_pad+h*tooltip.length
if(!param.do_selection&&1===num_pages&&input.mouseOver({x:x,y:tooltip_y1-vis_h,w:w,h:vis_h}))alpha=.15
var style=help_font_style
if(1!==alpha)style=glov_font.styleAlpha(style,alpha)
var y=tooltip_y1-eff_tooltip_pad
var ret=null
if(num_pages>1){ui.font.drawSizedAligned(help_font_style,text_x,y-=h,z+1,h,glov_font.ALIGN.HCENTER,text_w,0,"Page "+(tooltip_page+1)+" / "+num_pages)
var pos={x:x,y:y,w:w,h:h}
if(input.mouseUpEdge(pos))tooltip_page=(tooltip_page+1)%num_pages
else if(input.mouseOver(pos))ui.drawRect(x,y,x+w,y+h,z+.5,help_rollover_color)}for(var ii=tooltip.length-1;ii>=0;--ii){var line=tooltip[ii]
y-=h
var idx=line.indexOf(" ")
if("/"===line[0]&&-1!==idx&&param.do_selection){var cmd=line.slice(0,idx)
...
if(input.mouseUpEdge(_pos))ret=cmd.slice(1)
else if(input.mouseOver(_pos)){ui.drawRect(x,y,text_x+cmd_w+4,y+h,z+.5,help_rollover_color)
...
var pixel_scale=.5*ui.tooltip_panel_pixel_scale
...
if(!netClient().connected)this.addChatError("Cannot chat: Disconnected")
else if(!this.channel||!netSubs().loggedIn()&&!netSubs().allow_anon)this.addChatError("Cannot chat: Must be logged in")
else if(text.length>this.max_len)this.addChatError("Chat message too long")
else{var pak=this.channel.pak("chat")
pak.writeInt(flags)
pak.writeString(text)
pak.send(function(err,data){if(err)if("ERR_ECHO"===err){var _this2$channel,_this2$channel$data,_this2$channel$data$p,_this2$channel$data$p2,_this2$channel$data$p3
var roles=null==(_this2$channel=_this2.channel)?void 0:null==(_this2$channel$data=_this2$channel.data)?void 0:null==(_this2$channel$data$p=_this2$channel$data.public)?void 0:null==(_this2$channel$data$p2=_this2$channel$data$p.clients[netClientId()])?void 0:null==(_this2$channel$data$p3=_this2$channel$data$p2.ids)?void 0:_this2$channel$data$p3.roles
var style=_this2.classifyRole(roles,true)
_this2.onMsgChat({msg:text,style:style,id:netUserId(),client_id:netClientId(),display_name:netSubs().getDisplayName(),flags:flags})}else{_this2.addChatError(err)
if(!_this2.edit_text_entry.getText())_this2.edit_text_entry.setText(text)}})}}
ChatUI.prototype.setZOverride=function(z){this.z_override=z}
ChatUI.prototype.run=function(opts){var _this3=this
var UI_SCALE=ui.font_height/24
var border=(opts=opts||{}).border||this.border||8*UI_SCALE
var SPACE_ABOVE_ENTRY=border
var scroll_grow=opts.scroll_grow||0
if(netClient().disconnected&&!this.hide_disconnected_message)ui.font.drawSizedAligned(glov_font.style(null,{outline_width:2,outline_color:255,color:3709870335}),camera2d.x0(),this.disconnected_message_top?.8*engine.game_height:camera2d.y0(),Z.DEBUG,ui.font_height,this.disconnected_message_top?glov_font.ALIGN.HCENTER:glov_font.ALIGN.HVCENTER,camera2d.w(),.2*camera2d.h(),"Connection lost, attempting to reconnect ("+(netClient().timeSinceDisconnect()/1e3).toFixed(0)+")...")
if(!this.did_run_late)this.runLate()
this.did_run_late=false
var x0=void 0===opts.x?camera2d.x0():opts.x
var y0=void 0===opts.y?camera2d.y1()-this.h:opts.y
var y1=y0+this.h
var x=x0+border
var y=y1
var outer_w=this.w
var was_focused=this.isFocused()
var z=this.z_override||(was_focused?Z.CHAT_FOCUSED:Z.CHAT)
this.z_override=null
...
if(aspect>2)font_scale=4+4*min((aspect-2)/8,1)
font_height*=font_scale}var inner_w=outer_w-border+this.inner_width_adjust
this.setActiveSize(font_height,inner_w)
if(!hide_text_input){anything_visible=true
y-=border+font_height+1
if(!was_focused&&opts.pointerlock&&input.pointerLocked())ui.font.drawSizedAligned(this.styles.def,x,y,z+1,font_height,glov_font.ALIGN.HFIT,inner_w,0,"<Press Enter to chat>")
else{if(was_focused){var pressed_tab=!input.keyDown(input.KEYS.SHIFT)&&input.keyDownEdge(input.KEYS.TAB)
if(pressed_tab)this.focus()
var cur_text=this.edit_text_entry.getText()
...
if(1===autocomplete.length&&first.cname&&cmd_parse.canonical(cur_text.slice(1)).slice(0,first.cname.length)===first.cname)if(first.usage)auto_text=first.usage.split("\n")
else auto_text=[first.help]
else do_selection=true
var tooltip_y=y
var last_msg=this.msgs[this.msgs.length-1]
if(last_msg)if((msg=last_msg.msg)&&!(last_msg.flags&CHAT_FLAG_USERCHAT)&&"[error]"===msg.slice(0,7)){var numlines=last_msg.numlines
tooltip_y-=font_height*numlines+SPACE_ABOVE_ENTRY}var selected=drawHelpTooltip({x:x,y:tooltip_y,tooltip_width:max(inner_w,.8*engine.game_width),tooltip:auto_text,do_selection:do_selection,font_height:min(font_height,camera2d.w()/30)})
if(do_selection)if(pressed_tab||selected)this.edit_text_entry.setText("/"+(selected||first.cmd)+" ")}}}else this.history.resetPos()
if(input.keyDownEdge(input.KEYS.UP))this.edit_text_entry.setText(this.history.prev(cur_text))
if(input.keyDownEdge(input.KEYS.DOWN))this.edit_text_entry.setText(this.history.next(cur_text))
this.scroll_area.keyboardScroll()}var input_height=font_height
var input_width=inner_w-(opts.cuddly_scroll?this.scroll_area.barWidth()+1+border:border)
if(input.touch_mode&&!was_focused){y-=2*font_height
input_height=3*font_height
input_width=6*font_height}var res=this.edit_text_entry.run({x:x,y:y,w:input_width,font_height:input_height,pointer_lock:opts.pointerlock})
...
this.edit_text_entry.setText("")
if("/"===text[0]){if("/"===text[1])text=text.slice(1)
this.history.add(text)
if(netSubs())netSubs().serverLog("cmd",text)
this.cmdParse(text.slice(1),function(err){if(!err)return
if(_this3.volume_out)setTimeout(function(){return ui.playUISound("msg_out_err",_this3.volume_out)},max(0,1e3*_this3.msg_out_err_delay-(Date.now()-start_time)))
if(!_this3.edit_text_entry.getText())_this3.edit_text_entry.setText(text)
if(!is_focused)_this3.focus()})}else this.sendChat(0,text)
if(this.volume_out)ui.playUISound("msg_out",this.volume_out)
if(settings.chat_auto_unfocus){is_focused=false
...
var bracket_width=0
var name_width={}
var did_user_mouseover=false
function drawChatLine(msg,alpha){if(msg.hidden)return
var line=msg.msg_text
var numlines=msg.numlines
var is_url=do_scroll_area&&url_match&&matchAll(line,url_match)
var url_label=is_url=is_url&&1===is_url.length&&is_url[0]
if(is_url&&url_info){var m=is_url.match(url_info)
if(m)url_label=m[1]}var h=font_height*numlines
var text_w
var mouseover=false
if(do_scroll_area&&!input.mousePosIsTouch()&&(!msg.style||messageFromUser(msg)||is_url)){text_w=ui.font.getStringWidth(styles.def,font_height,line)
mouseover=input.mouseOver({x:x,y:y,w:min(text_w,wrap_w),h:h,peek:true})}var user_mouseover=false
var user_indent=0
var did_user_context=false
if(msg.flags&CHAT_FLAG_USERCHAT&&user_context_cb&&msg.id&&do_scroll_area){var nw=name_width[msg.display_name]
if(!nw)nw=name_width[msg.display_name]=ui.font.getStringWidth(styles.def,font_height,msg.display_name)
if(!(msg.flags&CHAT_FLAG_EMOTE)){if(!bracket_width)bracket_width=ui.font.getStringWidth(styles.def,font_height,"[]")
nw+=bracket_width}var pos_param={x:x,y:y,w:min(user_indent=nw,wrap_w),h:font_height,button:0,peek:true,z:z+.5,color:color_user_rollover}
...
if(self.user_id_mouseover===msg.id)ui.drawRect2(_extends({},pos_param,{color:color_same_user_rollover}))
if(user_mouseover){ui.drawRect2(pos_param)
did_user_mouseover=true
self.user_id_mouseover=msg.id}}}var click
if(is_url)click=link({x:x+user_indent,y:y,w:wrap_w-user_indent,h:h,url:is_url,internal:true})
var style
if(is_url)style=mouseover&&!user_mouseover?styles.link_hover:styles.link
else style=styles[msg.style]||styles.def
ui.font.drawSizedWrapped(glov_font.styleAlpha(style,alpha),x,y,z+1,wrap_w,self.indent,font_height,line)
if(mouseover&&(!do_scroll_area||y>self.scroll_area.getScrollPos()-font_height)&&(!msg.style||messageFromUser(msg)||is_url))ui.drawTooltip({x:x,y:y,z:Z.TOOLTIP,tooltip_above:true,tooltip_width:450*UI_SCALE,tooltip_pad:round(.5*ui.tooltip_pad),tooltip:is_url&&!user_mouseover?"Click to open "+url_label:"Received"+(msg.id?' from "'+msg.id+'"':"")+" at "+conciseDate(new Date(msg.timestamp))+"\nRight-click to copy message"+(user_mouseover?"\nClick to view user info":""),pixel_scale:.5*ui.tooltip_panel_pixel_scale})
click=click||input.click({x:x,y:y,w:wrap_w,h:h})
if(did_user_context)click=null
if(click)if(2===click.button)ui.provideUserString("Chat Text",is_url||line)
else if(is_url)self.cmdParseInternal("url "+url_label)
anything_visible=true}var now=Date.now()
...
for(ii=0;ii<this.msgs.length;++ii){var msg=this.msgs[ii]
var h=font_height*msg.numlines
if(y<=y_max&&y+h>=y_min)drawChatLine(msg,1)
y+=h}this.scroll_area.end(scroll_internal_h)
x=x_save
...
if(is_focused&&input.mouseDownEdge({peek:true})){spotUnfocus()
...
if(!alpha||_msg.quiet)break
if((numlines=_msg.numlines)>max_lines&&_ii2)break
max_lines-=numlines
y-=font_height*numlines
drawChatLine(_msg,alpha)}}if(!did_user_mouseover)self.user_id_mouseover=false
if(opts.pointerlock&&is_focused&&input.pointerLocked())input.pointerLockExit()
if(!anything_visible&&(ui.modal_dialog||ui.menu_up||hide_light))return
ui.drawRect(x0,y-border,x0+outer_w,y1,z,[.3,.3,.3,.8])}
ChatUI.prototype.setChannel=function(channel){var _this4=this
if(channel===this.channel)return
if(this.channel){if(!channel)this.addChat("Left channel "+this.channel.channel_id)
...
var here_map={}
var friends=[]
asyncParallel([function(next){channel.send("chat_get",null,function(err,data){if(!err&&data&&data.msgs&&data.msgs.length)chat_history=data
next()})},function(next){channel.onceSubscribe(function(data){var clients=data&&data.public&&data.public.clients
if(clients)for(var client_id in clients){var client=clients[client_id]
var user_id=client.ids&&client.ids.user_id
var already_in_list=false
if(user_id&&client.ids.display_name)if(here_map[user_id])already_in_list=true
else here_map[user_id]=client.ids.display_name
if(client_id===netClientId()||already_in_list)continue
if(client.ids)if(user_id&&isFriend(user_id))friends.push(client.ids.display_name||user_id||client_id)
else here.push(client.ids.display_name||user_id||client_id)}next()})}],function(){if(!_this4.channel)return
if(chat_history){var messages_pre=_this4.msgs.slice(0)
if(messages_pre.length)_this4.msgs=[]
for(var ii=0;ii<chat_history.msgs.length;++ii){var idx=(chat_history.idx+ii)%chat_history.msgs.length
var elem=chat_history.msgs[idx]
if(elem&&elem.msg){elem.quiet=true
if(here_map[elem.id])elem.display_name=here_map[elem.id]
_this4.onMsgChat(elem)}}if(messages_pre.length)_this4.msgs=_this4.msgs.concat(messages_pre)}_this4.addChat("Joined channel "+_this4.channel.channel_id,"join_leave")
if(here.length||friends.length){var msg=[]
if(here.length)if(here.length>10)msg.push("Other users already here: "+here.slice(0,10).join(", ")+" (and "+(here.length-10)+" more...)")
else msg.push("Other users already here: "+here.join(", "))
if(friends.length)msg.push("Friends already here: "+friends.join(", "))
_this4.addChatFiltered({msg:msg.join("\n"),style:"join_leave"})}})}
function chatUICreate(params){profanityStartup()
var chat_ui=new ChatUI(params)
function emote(str,resp_func){if(!str)return void resp_func(null,"Usage: /me does something.")
if(params.emote_cb)params.emote_cb(str)
chat_ui.sendChat(CHAT_FLAG_EMOTE,str)}cmd_parse.registerValue("volume_chat_joinleave",{type:cmd_parse.TYPE_FLOAT,label:"Join/Leave chat message volume",range:[0,1],get:function get(){return chat_ui.volume_join_leave},set:function set(v){return chat_ui.volume_join_leave=v},store:true})
...
},{"../common/util.js":99,"../common/vmath.js":101,"./camera2d.js":15,"./cmds.js":18,"./engine.js":23,"./font.js":32,"./input.js":42,"./link.js":44,"./local_storage.js":45,"./localization.js":46,"./net.js":50,"./scroll_area.js":61,"./settings.js":63,"./social.js":68,"./spot.js":71,"./ui.js":78,"./words/profanity.js":83,"assert":undefined,"glov-async":undefined}],17:[function(require,module,exports){
"use strict"
...
},{"../common/enums":92,"assert":undefined}],18:[function(require,module,exports){
"use strict"
...
},{"../common/cmd_parse.js":88,"../common/wscommon.js":103,"./engine.js":23,"./error_report.js":28,"./local_storage.js":45,"./net.js":50,"./shaders.js":65,"./textures.js":76}],19:[function(require,module,exports){
"use strict"
...
var _require2=require("./sprites.js"),spriteClipped=_require2.spriteClipped,spriteClipPause=_require2.spriteClipPause,spriteClipResume=_require2.spriteClipResume,spriteCreate=_require2.spriteCreate
var textures=require("./textures.js")
...
for(var i=0;i<HS_SIZE;i++){hsvToRGB(rgb,360*i/(HS_SIZE-1),sat,1)
data[idx++]=255*rgb[0]
data[idx++]=255*rgb[1]
data[idx++]=255*rgb[2]}}picker_sprite_hue_sat=spriteCreate({url:"cpicker_hs",width:HS_SIZE,height:HS_SIZE,format:textures.format.RGB8,data:data,filter_min:gl.LINEAR,filter_mag:gl.LINEAR,wrap_s:gl.CLAMP_TO_EDGE,wrap_t:gl.CLAMP_TO_EDGE})
data=new Uint8Array(32)
for(var ii=0;ii<data.length;++ii)data[ii]=255-255*ii/(data.length-1)
picker_sprite_val=spriteCreate({url:"cpicker_v",width:1,height:data.length,format:textures.format.R8,data:data,filter_min:gl.LINEAR,filter_mag:gl.LINEAR,wrap_s:gl.CLAMP_TO_EDGE,wrap_t:gl.CLAMP_TO_EDGE})}function colorPicker(param){var state=ui.getUIElemData("colorpicker",param,colorPickerAlloc)
var icon_h=param.icon_h||ui.button_height
var icon_w=param.icon_w||icon_h
var picker_h=param.picker_h||4*ui.button_height
var pad=param.pad||3
var x=param.x,y=param.y,z=param.z
if(!state.open)v3copy(state.rgba,param.color)
if(ui.buttonImage({x:x,y:y,z:z,w:icon_w,h:icon_h,img:ui.sprites.white,color:state.rgba}))if(!state.open)colorPickerOpen(state)
else state.open=false
var handled=ui.button_mouseover
if(state.open){var clip_pause=spriteClipped()
if(clip_pause)spriteClipPause()
if(!picker_sprite_hue_sat)initTextures()
z+=2
var x0=x+=icon_w+pad
var y0=y=min(y,camera2d.y1()-picker_h)
var hsv=state.hsv
var hue_sat_w=picker_h
var val_w=.1*picker_h
...
if(drag=input.drag(val_param)||input.click(val_param)){handled=true
...
var panel_param={x:x0-pad,y:y0-pad,w:x-x0+2*pad,h:picker_h+2*pad,z:z-1}
if(input.mouseOver(panel_param))handled=true
input.drag(panel_param)
ui.panel(panel_param)
if(clip_pause)spriteClipResume()
if(input.click({peek:true})||!handled&&input.mouseDownAnywhere())state.open=false}v3copy(param.color,state.rgba)}

},{"../common/util.js":99,"../common/vmath.js":101,"./camera2d.js":15,"./hsv.js":40,"./input.js":42,"./sprites.js":74,"./textures.js":76,"./ui.js":78}],20:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"../common/vmath.js":101,"./engine.js":23,"./geom.js":34,"./shaders.js":65,"./sprites.js":74,"./textures.js":76,"assert":undefined,"gl-mat4/lookAt":undefined}],21:[function(require,module,exports){
"use strict"
...
},{"../common/verify.js":100,"./camera2d.js":15,"./engine.js":23,"./input.js":42,"./localization.js":46,"./spot.js":71,"./ui.js":78,"assert":undefined}],22:[function(require,module,exports){
"use strict"
...
},{"../common/vmath.js":101,"./engine.js":23,"./framebuffer.js":33,"./geom.js":34,"./shaders.js":65,"./sprites.js":74,"./textures.js":76,"assert":undefined}],23:[function(require,module,exports){
"use strict"
...
},{"../common/perfcounters.js":96,"../common/util.js":99,"../common/verify.js":100,"../common/vmath.js":101,"./bootstrap.js":12,"./browser.js":13,"./build_ui.js":14,"./camera2d.js":15,"./client_config.js":17,"./cmds.js":18,"./effects.js":22,"./error_report.js":28,"./font.js":32,"./framebuffer.js":33,"./geom.js":34,"./input.js":42,"./local_storage.js":45,"./models.js":49,"./particles.js":52,"./perf.js":53,"./profiler.js":57,"./profiler_ui.js":58,"./settings.js":63,"./shader_debug_ui.js":64,"./shaders.js":65,"./sound.js":69,"./spot.js":71,"./sprites.js":74,"./textures.js":76,"./transition.js":77,"./ui.js":78,"./urlhash.js":80,"assert":undefined,"gl-mat3/fromMat4":undefined,"gl-mat4/copy":undefined,"gl-mat4/invert":undefined,"gl-mat4/multiply":undefined,"gl-mat4/perspective":undefined,"gl-mat4/transpose":undefined,"not_worker":undefined}],24:[function(require,module,exports){
"use strict"
exports.EntityBaseClient=void 0
function _inheritsLoose(subClass,superClass){subClass.prototype=Object.create(superClass.prototype)
_setPrototypeOf(subClass.prototype.constructor=subClass,superClass)}function _setPrototypeOf(o,p){return(_setPrototypeOf=Object.setPrototypeOf?Object.setPrototypeOf.bind():function _setPrototypeOf(o,p){o.__proto__=p
return o})(o,p)}var assert=require("assert")
var _glovClientEngine=require("./engine")
var engine=_glovClientEngine
var _glovCommonEntity_base_common=require("../common/entity_base_common")
var EntityBaseCommon=_glovCommonEntity_base_common.EntityBaseCommon
var EntityBaseClient=function(_EntityBaseCommon){_inheritsLoose(EntityBaseClient,_EntityBaseCommon)
function EntityBaseClient(ent_id,entity_manager){var _this;(_this=_EntityBaseCommon.call(this,ent_id,entity_manager)||this).fading_out=void 0
_this.fading_in=void 0
_this.seq_id=void 0
_this.data_overrides=void 0
_this.fade=void 0
_this.last_update_timestamp=void 0
_this.is_me=void 0
_this.fade=null
_this.fading_out=false
_this.fading_in=false
_this.data_overrides=[]
_this.seq_id=0
_this.last_update_timestamp=engine.frame_timestamp
_this.is_me=ent_id===entity_manager.my_ent_id
return _this}var _proto=EntityBaseClient.prototype
_proto.isMe=function isMe(){return this.is_me}
_proto.getData=function getData(field,deflt){var ret=this.data[field]
for(var ii=0;ii<this.data_overrides.length;++ii){var v=this.data_overrides[ii].data[field]
if(void 0!==v)if(null===v)ret=void 0
else ret=v}return null!=ret?ret:deflt}
_proto.setDataOverride=function setDataOverride(field,data){var field_start=this.getData(field)
var field_new=""+this.entity_manager.sub_id_prefix+ ++this.seq_id
data[field]=field_new
this.data_overrides.push({field:field,field_start:field_start,field_new:field_new,data:data})
return field_start}
_proto.postEntUpdate=function postEntUpdate(){var _this2=this
this.last_update_timestamp=engine.frame_timestamp
if(this.data_overrides.length){var by_field={}
this.data_overrides=this.data_overrides.filter(function(override){var field=override.field
if(by_field[field])return true
else if(_this2.data[field]===override.field_start)return by_field[field]=true
else return false})}}
_proto.actionPrepDataAssignments=function actionPrepDataAssignments(action_data,field,data_assignments){assert(!action_data.data_assignments)
assert(!action_data.predicate)
var expected_value=this.setDataOverride(field,data_assignments)
action_data.predicate={field:field,expected_value:expected_value}
if(data_assignments.client_only){var _action_data$data_ass
action_data.data_assignments=((_action_data$data_ass={})[field]=data_assignments[field],_action_data$data_ass)}else action_data.data_assignments=data_assignments}
_proto.handleActionResult=function handleActionResult(action_data,err,data){if(err){var predicate=action_data.predicate
if(predicate){var _field=predicate.field
var walk_id=predicate.expected_value
this.data_overrides=this.data_overrides.filter(function(override){if(override.field===_field)if(override.field_start===walk_id){walk_id=override.field_new
return false}return true})}}}
_proto.actionSend=function actionSend(action,resp_func){(action.ent=this).entity_manager.actionSendQueued(action,resp_func)}
_proto.applyBatchUpdate=function applyBatchUpdate(update,resp_func){var field=update.field,action_id=update.action_id,payload=update.payload,data_assignments=update.data_assignments
assert(data_assignments)
assert(!update.predicate)
var action_data={action_id:action_id,payload:payload}
this.actionPrepDataAssignments(action_data,field,data_assignments)
this.actionSend(action_data,resp_func)}
_proto.onDelete=function onDelete(reason){return 250}
_proto.onCreate=function onCreate(is_initial){return is_initial&&!this.entity_manager.received_ent_ready?0:250}
return EntityBaseClient}(EntityBaseCommon)
exports.EntityBaseClient=EntityBaseClient

},{"../common/entity_base_common":91,"./engine":23,"assert":undefined}],25:[function(require,module,exports){
"use strict"
exports.clientEntityManagerCreate=clientEntityManagerCreate
function _assertThisInitialized(self){if(void 0===self)throw new ReferenceError("this hasn't been initialised - super() hasn't been called")
return self}function _inheritsLoose(subClass,superClass){subClass.prototype=Object.create(superClass.prototype)
_setPrototypeOf(subClass.prototype.constructor=subClass,superClass)}function _setPrototypeOf(o,p){return(_setPrototypeOf=Object.setPrototypeOf?Object.setPrototypeOf.bind():function _setPrototypeOf(o,p){o.__proto__=p
return o})(o,p)}var assert=require("assert")
var _glovCommonEntity_base_common=require("../common/entity_base_common")
var EALF_HAS_ASSIGNMENTS=_glovCommonEntity_base_common.EALF_HAS_ASSIGNMENTS
var EALF_HAS_ENT_ID=_glovCommonEntity_base_common.EALF_HAS_ENT_ID
var EALF_HAS_PAYLOAD=_glovCommonEntity_base_common.EALF_HAS_PAYLOAD
var EALF_HAS_PREDICATE=_glovCommonEntity_base_common.EALF_HAS_PREDICATE
var EntityFieldEncoding=_glovCommonEntity_base_common.EntityFieldEncoding
var EntityFieldSpecial=_glovCommonEntity_base_common.EntityFieldSpecial
var EntityFieldSub=_glovCommonEntity_base_common.EntityFieldSub
var EntityUpdateCmd=_glovCommonEntity_base_common.EntityUpdateCmd
var _glovCommonTinyEvents=require("../common/tiny-events")
var EventEmitter=_glovCommonTinyEvents.EventEmitter
var _glovCommonUtil=require("../common/util")
var ridx=_glovCommonUtil.ridx
var _engine=require("./engine")
var engine=_engine
var _net=require("./net")
var netClientId=_net.netClientId
...
function entActionAppend(entity_manager,ent,pak,action_data){var action_id=action_data.action_id,ent_id=action_data.ent_id,predicate=action_data.predicate,self=action_data.self,payload=action_data.payload,data_assignments=action_data.data_assignments
var flags=0
if(predicate)flags|=EALF_HAS_PREDICATE
if(self);else flags|=EALF_HAS_ENT_ID
if(void 0!==payload)flags|=EALF_HAS_PAYLOAD
if(data_assignments)flags|=EALF_HAS_ASSIGNMENTS
pak.writeInt(flags)
pak.writeAnsiString(action_id)
if(flags&EALF_HAS_PREDICATE){assert(predicate)
pak.writeAnsiString(predicate.field)
pak.writeAnsiString(predicate.expected_value||"")}if(flags&EALF_HAS_ENT_ID){assert(ent_id)
pak.writeInt(ent_id)}if(flags&EALF_HAS_PAYLOAD)pak.writeJSON(payload)
if(flags&EALF_HAS_ASSIGNMENTS){var field_defs_by_name=entity_manager.field_defs_by_name
assert(field_defs_by_name)
for(var key in data_assignments){var field_def=field_defs_by_name[key]
assert(field_def)
var field_id=field_def.field_id,sub=field_def.sub,default_value=field_def.default_value,encoder=field_def.encoder
assert(!sub)
var value=data_assignments[key]
if(value===default_value||null===value&&void 0===default_value){pak.writeInt(EntityFieldSpecial.Default)
pak.writeInt(field_id)}else{pak.writeInt(field_id)
encoder(ent,pak,value)}}pak.writeInt(EntityFieldSpecial.Terminate)}}var ClientEntityManagerImpl=function(_EventEmitter){_inheritsLoose(ClientEntityManagerImpl,_EventEmitter)
function ClientEntityManagerImpl(options){var _this;(_this=_EventEmitter.call(this)||this).my_ent_id=void 0
_this.on_broadcast=void 0
_this.EntityCtor=void 0
_this.channel=void 0
_this.client_id=void 0
_this.subscription_id=void 0
_this.sub_id_prefix=void 0
_this.entities=void 0
_this.fading_ents=void 0
_this.field_defs=void 0
_this.field_defs_by_name=void 0
_this.field_decoders=void 0
_this.field_encoders=void 0
_this.received_ent_ready=void 0
_this.received_ent_start=void 0
_this.frame_wall_time=void 0
_this.action_list_queue=null
assert(options.channel_type)
netSubs().onChannelMsg(options.channel_type,"ent_update",_this.onEntUpdate.bind(_assertThisInitialized(_this)))
netSubs().onChannelMsg(options.channel_type,"ent_broadcast",_this.onBroadcast.bind(_assertThisInitialized(_this)))
netSubs().onChannelMsg(options.channel_type,"ent_start",_this.onEntStart.bind(_assertThisInitialized(_this)))
netSubs().onChannelMsg(options.channel_type,"ent_ready",_this.onEntReady.bind(_assertThisInitialized(_this)))
netSubs().onChannelMsg(options.channel_type,"ent_id_change",_this.onEntIdChange.bind(_assertThisInitialized(_this)))
netSubs().onChannelEvent(options.channel_type,"subscribe",_this.onChannelSubscribe.bind(_assertThisInitialized(_this)))
_this.reinit(options)
_this.field_decoders=_this.EntityCtor.field_decoders
_this.field_encoders=_this.EntityCtor.field_encoders
_this.frame_wall_time=walltime()
return _this}var _proto=ClientEntityManagerImpl.prototype
_proto.reinit=function reinit(options){this.deinit()
this.EntityCtor=options.EntityCtor||this.EntityCtor
this.on_broadcast=options.on_broadcast||this.on_broadcast
this.channel=options.channel
this.reinitInternal()}
_proto.reinitInternal=function reinitInternal(){this.entities={}
this.fading_ents=[]
this.my_ent_id=0
this.received_ent_ready=false
this.received_ent_start=false
this.sub_id_prefix="::"}
_proto.deinit=function deinit(){this.received_ent_start=false}
_proto.finalizeDelete=function finalizeDelete(ent_id){this.emit("ent_delete",ent_id)
delete this.entities[ent_id]}
_proto.tick=function tick(){this.frame_wall_time=max(this.frame_wall_time,walltime())
if(this.fading_ents)for(var ii=this.fading_ents.length-1;ii>=0;--ii){var elem=this.fading_ents[ii]
elem.countdown-=engine.frame_dt
var ent_id=elem.ent_id
if(elem.countdown<=0){if(elem.is_out)this.finalizeDelete(ent_id)
else{var _ent=this.entities[ent_id]
assert(_ent)
assert(_ent.fading_in)
_ent.fading_in=false
_ent.fade=null}ridx(this.fading_ents,ii)}else{var _ent2=this.entities[ent_id]
assert(_ent2)
if(elem.is_out)_ent2.fade=min(1,elem.countdown/elem.countdown_max)
else _ent2.fade=1-elem.countdown/elem.countdown_max}}}
_proto.getFrameWallTime=function getFrameWallTime(){return this.frame_wall_time}
_proto.receivedEntStart=function receivedEntStart(){return this.received_ent_start}
_proto.receivedEntReady=function receivedEntReady(){return this.received_ent_ready}
_proto.onChannelSubscribe=function onChannelSubscribe(data){this.reinitInternal()
this.client_id=netClientId()
this.subscription_id=data.sub_id||this.client_id
this.sub_id_prefix=this.subscription_id+":"
this.emit("subscribe",data)}
_proto.onBroadcast=function onBroadcast(data){if(!this.received_ent_start)return
this.on_broadcast(data)}
_proto.fadeInEnt=function fadeInEnt(ent,time){assert(!ent.fading_out)
assert(!ent.fading_in)
ent.fading_in=true
this.fading_ents.push({is_out:false,ent_id:ent.id,countdown:time,countdown_max:time})}
_proto.deleteEntityInternal=function deleteEntityInternal(ent_id,reason){var ent=this.entities[ent_id]
assert(ent)
assert(!ent.fading_out)
ent.fading_out=true
var countdown_max=ent.onDelete(reason)
if(ent.fading_in){ent.fading_in=false
for(var ii=0;ii<this.fading_ents.length;++ii){var fade=this.fading_ents[ii]
if(fade.ent_id===ent_id){if(countdown_max){fade.is_out=true
fade.countdown=round((ent.fade||0)*countdown_max)
fade.countdown_max=countdown_max}else{ridx(this.fading_ents,ii)
this.finalizeDelete(ent_id)}return}}assert(false)}if(countdown_max)this.fading_ents.push({is_out:true,ent_id:ent_id,countdown:countdown_max,countdown_max:countdown_max})
else this.finalizeDelete(ent_id)}
_proto.getEntityForUpdate=function getEntityForUpdate(ent_id){var ent=this.entities[ent_id]
if(!ent)ent=this.entities[ent_id]=new this.EntityCtor(ent_id,this)
if(ent.fading_out){ent.fading_out=false
ent.fade=null
for(var jj=0;jj<this.fading_ents.length;++jj)if(this.fading_ents[jj].ent_id===ent_id){ridx(this.fading_ents,jj)
break}}return ent}
_proto.readDiffFromPacket=function readDiffFromPacket(ent_id,pak){var field_defs=this.field_defs,field_decoders=this.field_decoders
assert(field_defs)
var ent=this.getEntityForUpdate(ent_id)
var data=ent.data
var field_id
while(field_id=pak.readInt()){var do_default=field_id===EntityFieldSpecial.Default
if(do_default)field_id=pak.readInt()
var field_def=field_defs[field_id]
if(!field_def)assert(field_def,'Missing field_def in server-provided schema for field#"'+field_id+'"')
var default_value=field_def.default_value,encoding=field_def.encoding,field_name=field_def.field_name,sub=field_def.sub
var decoder=field_decoders[encoding]
if(!decoder)assert(decoder,"Missing decoder for type "+field_def.encoding)
if(sub){assert(!do_default)
var sub_value=data[field_name]
if(sub===EntityFieldSub.Array){if(!sub_value)sub_value=data[field_name]=[]
assert(Array.isArray(sub_value))
var index=void 0
while(index=pak.readInt())if(-1===index)sub_value.length=pak.readInt()
else{var new_value=decoder(pak,sub_value[index-1])
sub_value[index-1]=new_value}}else{if(!sub_value)sub_value=data[field_name]={}
assert(sub_value&&"object"===typeof sub_value&&!Array.isArray(sub_value))
var sub_obj=sub_value
var key=void 0
while(key=pak.readAnsiString()){var _new_value=decoder(pak,sub_obj[key])
if(void 0===_new_value)delete sub_obj[key]
else sub_obj[key]=_new_value}}}else{var _old_value2=data[field_name]
var _new_value2=do_default?default_value:decoder(pak,_old_value2)
if(void 0===_new_value2)delete data[field_name]
else data[field_name]=_new_value2}}ent.postEntUpdate()
this.emit("ent_update",ent.id)}
_proto.initSchema=function initSchema(schema){var field_defs=[null]
var field_defs_by_name={}
var field_encoders=this.field_encoders
for(var ii=0;ii<schema.length;++ii){var ser_def=schema[ii]
var idx=ii+EntityFieldSpecial.MAX
var encoding=ser_def.e||EntityFieldEncoding.JSON
var encoder=field_encoders[encoding]
assert(encoder)
var def=field_defs[idx]={encoding:encoding,encoder:encoder,default_value:ser_def.d,sub:ser_def.s||EntityFieldSub.None,field_name:ser_def.n,field_id:idx}
field_defs_by_name[def.field_name]=def}this.field_defs=field_defs
this.field_defs_by_name=field_defs_by_name}
_proto.initializeNewFullEnt=function initializeNewFullEnt(ent){var field_defs=this.field_defs
assert(field_defs)
var data=ent.data
for(var ii=0;ii<field_defs.length;++ii){var def=field_defs[ii]
if(!def)continue
var default_value=def.default_value,field_name=def.field_name
if(void 0!==default_value&&default_value!==data[field_name])data[field_name]=default_value}}
_proto.onEntUpdate=function onEntUpdate(pak){if(!this.received_ent_start){pak.pool()
return}var cmd
var is_initial=false
while(cmd=pak.readU8())switch(cmd){case EntityUpdateCmd.Full:var ent_id=pak.readInt()
var _ent3=this.getEntityForUpdate(ent_id)
this.initializeNewFullEnt(_ent3)
this.readDiffFromPacket(ent_id,pak)
var fade_in_time=_ent3.onCreate(is_initial)
if(fade_in_time)this.fadeInEnt(_ent3,fade_in_time)
break
case EntityUpdateCmd.Diff:var _ent_id=pak.readInt()
this.readDiffFromPacket(_ent_id,pak)
break
case EntityUpdateCmd.Delete:var _ent_id2=pak.readInt()
var reason=pak.readAnsiString()
this.deleteEntityInternal(_ent_id2,reason)
break
case EntityUpdateCmd.Event:var _data=pak.readJSON()
this.onBroadcast(_data)
break
case EntityUpdateCmd.Schema:var schema=pak.readJSON()
this.initSchema(schema)
break
case EntityUpdateCmd.IsInitialList:is_initial=true
break
default:assert(false,"Unexpected EntityUpdateCmd: "+cmd)}}
_proto.onEntReady=function onEntReady(){if(this.received_ent_start){this.received_ent_ready=true
this.emit("ent_ready")}}
_proto.isReady=function isReady(){return this.received_ent_ready}
_proto.onEntStart=function onEntStart(data){if(data.sub_id===this.subscription_id){this.my_ent_id=data.ent_id
this.received_ent_start=true
this.emit("ent_start")}}
_proto.onEntIdChange=function onEntIdChange(data){if(data.sub_id===this.subscription_id)this.my_ent_id=data.ent_id}
_proto.checkNet=function checkNet(){if(netDisconnected()||!this.channel||!this.received_ent_ready)return true
if(netClientId()!==this.client_id)return true
return false}
_proto.handleActionListResult=function handleActionListResult(action_list,resp_list,err,resp){resp=resp||[]
for(var ii=0;ii<resp_list.length;++ii){var this_resp=resp[ii]||{}
var this_err=err||this_resp.err
var this_data=this_resp.data
var action_data=action_list[ii]
var _ent4=action_data.ent
_ent4.handleActionResult(action_data,this_err,this_data)
var resp_func=resp_list[ii]
if(resp_func)resp_func(this_err||null,this_data)
else{var predicate=action_data.predicate,data_assignments=action_data.data_assignments
if(predicate&&data_assignments){var field=predicate.field,expected_value=predicate.expected_value
if("ERR_FIELD_MISMATCH"===this_err||"ERR_INVALID_ENT_ID"===this_err){console.log("Received (not unexpected) error "+this_err+" applying batch update "+field+"="+expected_value+"->"+data_assignments[field]+" to ent:"+_ent4.id)
return}}if(this_err)throw this_err}}}
_proto.actionListFlush=function actionListFlush(){if(!this.action_list_queue)return
assert(this.channel)
var _this$action_list_que=this.action_list_queue,action_list=_this$action_list_que.action_list,resp_list=_this$action_list_que.resp_list
this.action_list_queue=null
var pak=this.channel.pak("ent_action_list")
pak.writeInt(action_list.length)
for(var ii=0;ii<action_list.length;++ii){var action_data=action_list[ii]
var _ent5=action_data.ent
if(_ent5.id===this.my_ent_id)action_data.self=true
else action_data.ent_id=_ent5.id
entActionAppend(this,_ent5,pak,action_data)}pak.send(this.handleActionListResult.bind(this,action_list,resp_list))}
_proto.actionSendQueued=function actionSendQueued(action,resp_func){if(!this.action_list_queue)this.action_list_queue={action_list:[],resp_list:[]}
this.action_list_queue.action_list.push(action)
this.action_list_queue.resp_list.push(resp_func)}
_proto.getEnt=function getEnt(ent_id){return this.entities[ent_id]}
_proto.hasMyEnt=function hasMyEnt(){return Boolean(this.my_ent_id&&this.getEnt(this.my_ent_id))}
_proto.isEntless=function isEntless(){return!this.my_ent_id}
_proto.getMyEntID=function getMyEntID(){return this.my_ent_id}
_proto.getMyEnt=function getMyEnt(){assert(this.my_ent_id)
var ent=this.getEnt(this.my_ent_id)
assert(ent)
return ent}
_proto.entitiesFind=function entitiesFind(predicate,skip_fading_out){var entities=this.entities
var ret=[]
for(var ent_id_string in entities){var _ent6=entities[ent_id_string]
if(_ent6.fading_out&&skip_fading_out)continue
if(!predicate(_ent6))continue
ret.push(_ent6)}return ret}
return ClientEntityManagerImpl}(EventEmitter)
function clientEntityManagerCreate(options){return new ClientEntityManagerImpl(options)}

},{"../common/entity_base_common":91,"../common/tiny-events":98,"../common/util":99,"./engine":23,"./net":50,"./walltime.js":81,"assert":undefined}],26:[function(require,module,exports){
"use strict"
exports.PerEntData=void 0
exports.entityPositionManagerCreate=entityPositionManagerCreate
var assert=require("assert")
var _glovClientEngine=require("./engine")
var engine=_glovClientEngine
var _glovClientEngine2=require("./engine")
var getFrameTimestamp=_glovClientEngine2.getFrameTimestamp
var _glovCommonUtil=require("../common/util")
var sign=_glovCommonUtil.sign
var _perf_net=require("./perf_net")
var registerPingProvider=_perf_net.registerPingProvider
var abs=Math.abs,floor=Math.floor,max=Math.max,PI=Math.PI,sqrt=Math.sqrt
var TWO_PI=2*PI
var EPSILON=.01
function defaultErrorHandler(err){if("ERR_FAILALL_DISCONNECT"!==err)throw err}var PerEntData=function PerEntData(ent_pos_manager){this.pos=void 0
...
this.net_anim_state={}
this.anim_state={}}
exports.PerEntData=PerEntData
var EntityPositionManagerImpl=function(){function EntityPositionManagerImpl(options){this.per_ent_data=void 0
...
this.entless_send_time=options.entless_send_time||1e3
this.window=options.window||200
this.snap_factor=options.snap_factor||1
...
this.temp_delta=this.vec()}var _proto=EntityPositionManagerImpl.prototype
_proto.reinit=function reinit(){this.per_ent_data={}
this.sends_to_ignore=0
this.last_send={pos:this.vec(-1),anim_state:{},sending:false,send_time:0,time:0,hrtime:0}}
_proto.handleEntDelete=function handleEntDelete(ent_id){delete this.per_ent_data[ent_id]}
_proto.handleSubscribe=function handleSubscribe(){this.reinit()}
_proto.getPos=function getPos(ent_id){var ped=this.per_ent_data[ent_id]
if(!ped)return null
return ped.pos}
_proto.getPED=function getPED(ent_id){return this.per_ent_data[ent_id]}
_proto.vec=function vec(fill){var r=new Float64Array(this.n)
if(fill)for(var ii=0;ii<this.n;++ii)r[ii]=fill
return r}
_proto.vcopy=function vcopy(dst,src){for(var ii=0;ii<this.n;++ii)dst[ii]=src[ii]
return dst}
_proto.arr=function arr(vec){var arr=new Array(this.n)
for(var ii=0;ii<this.n;++ii)arr[ii]=vec[ii]
return arr}
_proto.arrPos=function arrPos(vec){var arr=new Array(this.dim_pos)
for(var ii=0;ii<this.dim_pos;++ii)arr[ii]=vec[ii]
return arr}
_proto.vsame=function vsame(a,b){for(var ii=0;ii<this.n;++ii)if(abs(a[ii]-b[ii])>EPSILON)return false
return true}
_proto.vsamePos=function vsamePos(a,b){for(var ii=0;ii<this.dim_pos;++ii)if(abs(a[ii]-b[ii])>EPSILON)return false
return true}
_proto.vlength=function vlength(a){var r=0
for(var ii=0;ii<this.n;++ii){var d=a[ii]
r+=d*d}return sqrt(r)}
_proto.vdist=function vdist(a,b){this.vsub(this.temp_vec,a,b)
...
_proto.vsub=function vsub(dst,a,b){for(var ii=0;ii<this.n;++ii)dst[ii]=a[ii]-b[ii]
return dst}
_proto.vscale=function vscale(dst,a,scalar){for(var ii=0;ii<this.n;++ii)dst[ii]=a[ii]*scalar
return dst}
_proto.stateDiff=function stateDiff(a,b){var anim_state_defs=this.anim_state_defs
var diff=null
for(var key in anim_state_defs){var def=anim_state_defs[key]
if(a[key]!==b[key]&&(!def.equal||!def.equal(a[key],b[key])))(diff=diff||{})[key]=true}return diff}
_proto.updateMyPos=function updateMyPos(character_pos,anim_state,force){var _this=this
var entless=this.entity_manager.isEntless()
if(!entless&&!this.entity_manager.hasMyEnt())return
var pos_diff=entless?!this.vsamePos(character_pos,this.last_send.pos):!this.vsame(character_pos,this.last_send.pos)
var state_diff=!entless&&this.stateDiff(anim_state,this.last_send.anim_state)
if(pos_diff||state_diff){var now=getFrameTimestamp()
var send_time=entless?this.entless_send_time:this.send_time
if(!this.last_send.sending&&(!this.last_send.time||now-this.last_send.time>send_time)||force){if(this.last_send.sending)++this.sends_to_ignore
...
if((speed=this.vdist(this.last_send.pos,character_pos)/time)<.001)speed=0}this.last_send.send_time=now
this.vcopy(this.last_send.pos,character_pos)
var data_assignments={}
if(pos_diff){data_assignments.pos=entless?this.arrPos(this.last_send.pos):this.arr(this.last_send.pos)
if(!entless)data_assignments.speed=speed}if(state_diff)for(var key in state_diff){data_assignments[key]=anim_state[key]
this.last_send.anim_state[key]=anim_state[key]}var handle_resp=function handle_resp(err){if(!_this.sends_to_ignore){_this.last_send.sending=false
var end=getFrameTimestamp()
var round_trip=engine.hrnow()-_this.last_send.hrtime
_this.ping_time=round_trip
_this.ping_time_time=end
if(round_trip>send_time)_this.last_send.time=end}else--_this.sends_to_ignore
if(err)return _this.error_handler(err)}
if(this.entity_manager.isEntless()){assert(this.entity_manager.channel)
this.entity_manager.channel.send("move",data_assignments,handle_resp)}else this.entity_manager.getMyEnt().actionSend({action_id:"move",data_assignments:data_assignments},handle_resp)}}else if(!this.last_send.sending)this.last_send.send_time=this.last_send.time=getFrameTimestamp()}
_proto.otherEntityChanged=function otherEntityChanged(ent_id){var anim_state_defs=this.anim_state_defs
var ent=this.entity_manager.getEnt(ent_id)
assert(ent)
var ent_data=ent.data
var ped=this.per_ent_data[ent_id]
if(!ped){ped=this.per_ent_data[ent_id]=new PerEntData(this)
for(var key in anim_state_defs)ped.anim_state[key]=ent_data[key]
this.vcopy(ped.pos,ent_data.pos)}for(var _key in anim_state_defs)ped.net_anim_state[_key]=ent_data[_key]
if(!this.vsame(ped.net_pos,ent_data.pos)){this.vcopy(ped.net_pos,ent_data.pos)
ped.net_speed=ent_data.speed
for(var ii=0;ii<this.dim_rot;++ii){var jj=this.dim_pos+ii
while(ped.pos[jj]>ped.net_pos[jj]+PI)ped.pos[jj]-=TWO_PI
while(ped.pos[jj]<ped.net_pos[jj]-PI)ped.pos[jj]+=TWO_PI}var delta=this.vsub(this.temp_delta,ped.net_pos,ped.pos)
var dist=this.vlength(delta)
if(dist>0){var time_to_dest=dist/ped.net_speed
if(time_to_dest<this.send_time+this.window)this.vscale(ped.impulse,delta,ped.net_speed/dist)
else if(time_to_dest<this.send_time+this.window*this.smooth_windows){var old_speed=this.vlength(ped.impulse)
var specified_speed=ped.net_speed
var new_speed=max(specified_speed*this.smooth_factor,old_speed)
this.vscale(ped.impulse,delta,new_speed/dist)}else this.vscale(ped.impulse,delta,1/(this.send_time+this.window*this.snap_factor))}}}
_proto.updateOtherEntity=function updateOtherEntity(ent_id,dt){var ped=this.per_ent_data[ent_id]
if(!ped)return null
var stopped=true
for(var ii=0;ii<this.n;++ii)if(ped.impulse[ii]){var delta_old=ped.net_pos[ii]-ped.pos[ii]
var delta_old_sign=sign(delta_old)
ped.pos[ii]+=ped.impulse[ii]*dt
var delta_new=ped.net_pos[ii]-ped.pos[ii]
if(sign(delta_new)!==delta_old_sign){ped.pos[ii]=ped.net_pos[ii]
ped.impulse[ii]=0}else if(ii<this.dim_pos&&abs(ped.impulse[ii])>.001)stopped=false}var anim_state_defs=this.anim_state_defs
for(var key in anim_state_defs)if(ped.anim_state[key]!==ped.net_anim_state[key]){var def=anim_state_defs[key]
var apply_change=true
if(def.stopped&&!stopped){var cur_is_move=!def.stopped(ped.anim_state[key])
var new_is_idle=def.stopped(ped.net_anim_state[key])
if(cur_is_move&&new_is_idle)apply_change=false}if(apply_change)ped.anim_state[key]=ped.net_anim_state[key]}return ped}
_proto.getPing=function getPing(){var max_age=2e3
if(!this.ping_time_time)return null
var age=getFrameTimestamp()-this.ping_time_time
if(age>max_age)return null
return{ping:this.ping_time,fade:1-age/max_age}}
return EntityPositionManagerImpl}()
function entityPositionManagerCreate(options){var ret=new EntityPositionManagerImpl(options)
registerPingProvider(ret.getPing.bind(ret))
return ret}

},{"../common/util":99,"./engine":23,"./perf_net":54,"assert":undefined}],27:[function(require,module,exports){
"use strict"
...
},{"./client_config":17,"./cmds":18,"./net":50,"./urlhash":80,"assert":undefined}],28:[function(require,module,exports){
"use strict"
...
else delete error_report_details[key]}function errorReportSetDynamicDetails(key,fn){error_report_dynamic_details[key]=fn}errorReportSetDetails("build","1673321730282")
...
},{"./client_config.js":17,"./environments":27,"./fetch.js":30}],29:[function(require,module,exports){
"use strict"
...
},{"../common/enums.js":92,"../common/util.js":99,"./error_report.js":28,"./local_storage.js":45,"./social.js":68,"./urlhash.js":80}],30:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"assert":undefined}],31:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"../common/vmath.js":101,"./camera2d.js":15,"./engine.js":23,"./geom.js":34,"./localization.js":46,"./shaders.js":65,"./sprites.js":74,"./textures.js":76,"assert":undefined}],33:[function(require,module,exports){
"use strict"
...
},{"./browser.js":13,"./cmds.js":18,"./effects.js":22,"./engine.js":23,"./perf.js":53,"./settings.js":63,"./textures.js":76,"assert":undefined}],34:[function(require,module,exports){
"use strict"
...
},{"./cmds.js":18,"./engine.js":23,"./perf.js":53,"./settings.js":63,"./shaders.js":65,"assert":undefined}],35:[function(require,module,exports){
"use strict"
...
},{"./decode-utf8.js":35,"./gltf-type-utils.js":36,"./unpack-binary-json.js":38,"./unpack-glb-buffers.js":39,"assert":undefined}],38:[function(require,module,exports){
"use strict"
...
},{"./gltf-type-utils.js":36,"assert":undefined}],40:[function(require,module,exports){
"use strict"
...
var mn=min(r,g,b)
var mx=max(r,g,b)
var delta=(out[2]=mx)-mn
if(0!==delta)out[1]=delta/mx
else{out[1]=0
out[0]=0
return}if(r===mx)out[0]=(g-b)/delta
else if(g===mx)out[0]=2+(b-r)/delta
else out[0]=4+(r-g)/delta
out[0]*=60
if(out[0]<0)out[0]+=360}
exports.hsvToRGB=function hsvToRGB(out,h,s,v){if(0===s){out[0]=out[1]=out[2]=v
return out}if((h/=60)>=6)h-=6
var i=floor(h)
var f=h-i
var p=v*(1-s)
var q=v*(1-s*f)
...
},{"../common/util.js":99,"../common/vmath.js":101,"./browser.js":13,"./camera2d.js":15,"./cmds.js":18,"./engine.js":23,"./in_event.js":41,"./input_constants":43,"./local_storage.js":45,"./pointer_lock.js":55,"./settings.js":63,"./sound.js":69,"./spot.js":71,"assert":undefined}],43:[function(require,module,exports){
"use strict"
...
},{"./camera2d.js":15,"./engine.js":23,"./font.js":32,"./in_event.js":41,"./input.js":42,"./settings.js":63,"./spot.js":71,"./ui.js":78,"assert":undefined}],45:[function(require,module,exports){
"use strict"
...
},{"assert":undefined}],46:[function(require,module,exports){
"use strict"
...
},{"../common/vmath.js":101,"./engine.js":23,"./fetch.js":30,"./geom.js":34,"./glb/gltf-type-utils.js":36,"./glb/parser.js":37,"./shaders.js":65,"./textures.js":76,"./webfs.js":82,"assert":undefined}],50:[function(require,module,exports){
"use strict"
...
subs.tick(dt)})}var build_timestamp_string=new Date(Number("1673321730282")).toISOString().replace("T"," ").slice(5,-8)
...
},{"../common/packet.js":95,"../common/wscommon.js":103,"./filewatch.js":31,"./subscription_manager.js":75,"./wsclient.js":84}],51:[function(require,module,exports){
"use strict"
exports.create=create
...
var _net=require("./net")
var net=_net
var _net2=require("./net")
var netDisconnected=_net2.netDisconnected
var _perf_net=require("./perf_net")
var registerPingProvider=_perf_net.registerPingProvider
...
var field=m[2]
if("pos"===field)this.otherClientPosChanged(client_id)
if(this.on_client_change){var pcd=this.per_client_data[client_id]
if(pcd)this.on_client_change(pcd,field)}}if(!mod_value)if(m=mod_key.match(/^public\.clients\.([^.]+)$/))delete this.per_client_data[m[1]]}else if(data&&data.public&&data.public.clients)for(var _client_id in data.public.clients)if(data.public.clients[_client_id].pos)this.otherClientPosChanged(_client_id)}
NetPositionManager.prototype.vec=function(fill){var r=new Float64Array(this.n)
if(fill)for(var ii=0;ii<this.n;++ii)r[ii]=fill
return r}
NetPositionManager.prototype.vcopy=function(dst,src){for(var ii=0;ii<this.n;++ii)dst[ii]=src[ii]
return dst}
NetPositionManager.prototype.arr=function(vec){var arr=new Array(this.n)
for(var ii=0;ii<this.n;++ii)arr[ii]=vec[ii]
return arr}
NetPositionManager.prototype.vsame=function(a,b){for(var ii=0;ii<this.n;++ii)if(abs(a[ii]-b[ii])>EPSILON)return false
return true}
...
var d=abs(this.temp_vec[jj])
if(d>PI)this.temp_vec[jj]=d-floor((d+PI)/TWO_PI)*TWO_PI}return this.vlength(this.temp_vec)}
NetPositionManager.prototype.vsub=function(dst,a,b){for(var ii=0;ii<this.n;++ii)dst[ii]=a[ii]-b[ii]
return dst}
...
if(!this.default_pos)this.default_pos=this.vec()
if(!this.temp_vec)this.temp_vec=this.vec()
if(!this.temp_delta)this.temp_delta=this.vec()
this.channel=options.channel
...
if(new_pos)npm.vcopy(npm.last_send.pos,new_pos)}NetPositionManager.prototype.checkNet=function(on_pos_set_cb){if(netDisconnected()||!this.channel||!this.channel.data.public)return true
if(net.client.id!==this.client_id)return true
var me=this.channel.getChannelData("public.clients."+this.client_id,{})
if(!me.pos||!me.pos.cur||"number"!==typeof me.pos.cur[0]){if(this.ever_received_character);else syncPosWithCaller(this,on_pos_set_cb)
this.channel.setChannelData("public.clients."+this.client_id+".pos",{cur:this.arr(this.last_send.pos)})
...
if(!this.last_send.sending&&(!this.last_send.time||now-this.last_send.time>this.send_time)||force){if(this.last_send.sending)++this.sends_to_ignore
else this.last_send.sending=true
this.last_send.time=now
...
var round_trip=glov_engine.hrnow()-_this.last_send.hrtime
_this.ping_time=round_trip
_this.ping_time_time=end
if(round_trip>_this.send_time)_this.last_send.time=end}else--_this.sends_to_ignore})}}}
NetPositionManager.prototype.getPing=function(){var max_age=2e3
if(!this.ping_time_time)return null
var age=glov_engine.getFrameTimestamp()-this.ping_time_time
if(age>max_age)return null
return{ping:this.ping_time,fade:1-age/max_age}}
...
if(!client_pos||!client_pos.cur||"number"!==typeof client_pos.cur[0])return
var pcd=this.per_client_data[client_id]
if(!pcd){(pcd=this.per_client_data[client_id]={}).pos=this.vcopy(this.vec(),client_pos.cur)
pcd.net_speed=0
pcd.net_pos=this.vec()
pcd.impulse=this.vec()
pcd.net_state="idle_down"
pcd.anim_state="idle_down"}if(client_pos.state)pcd.net_state=client_pos.state
this.vcopy(pcd.net_pos,client_pos.cur)
pcd.net_speed=client_pos.speed
for(var ii=0;ii<this.dim_rot;++ii){var jj=this.dim_pos+ii
while(pcd.pos[jj]>pcd.net_pos[jj]+PI)pcd.pos[jj]-=TWO_PI
while(pcd.pos[jj]<pcd.net_pos[jj]-PI)pcd.pos[jj]+=TWO_PI}var delta=this.vsub(this.temp_delta,pcd.net_pos,pcd.pos)
var dist=this.vlength(delta)
if(dist>0){var time_to_dest=dist/pcd.net_speed
if(time_to_dest<this.send_time+this.window)this.vscale(pcd.impulse,delta,pcd.net_speed/dist)
else if(time_to_dest<this.send_time+this.window*this.smooth_windows){var old_speed=this.vlength(pcd.impulse)
...
if(util.sign(delta_new)!==delta_old_sign){pcd.pos[ii]=pcd.net_pos[ii]
pcd.impulse[ii]=0}else if(ii<this.dim_pos&&pcd.impulse[ii]>.01)stopped=false}if(this.on_pos_update)this.on_pos_update(client_id,pcd.pos)
var cur_is_run="f"===pcd.anim_state[0]||"w"===pcd.anim_state[0]
var new_is_idle="i"===pcd.net_state[0]
...
function create(options){var ret=new NetPositionManager(options)
registerPingProvider(ret.getPing.bind(ret))
return ret}function ScalarInterpolator(tick_time){this.tick_time=1.25*tick_time
this.reset()}ScalarInterpolator.prototype.reset=function(){this.value=void 0
this.target_value=void 0
this.vel=0}
ScalarInterpolator.prototype.update=function(dt,new_value){if(void 0===this.value){this.value=new_value
this.target_value=new_value
return}if(new_value!==this.target_value){this.vel=(new_value-this.value)/this.tick_time
this.target_value=new_value}if(this.value!==this.target_value)if(this.vel>0)this.value=min(this.value+this.vel*dt,this.target_value)
...
},{"../common/util":99,"./engine":23,"./net":50,"./perf_net":54}],52:[function(require,module,exports){
"use strict"
exports.create=create
...
},{"../common/vmath.js":101,"./sprites.js":74,"./textures.js":76,"assert":undefined}],53:[function(require,module,exports){
"use strict"
...
},{"../common/perfcounters.js":96,"../common/vmath.js":101,"./camera2d.js":15,"./cmds.js":18,"./engine.js":23,"./font.js":32,"./input.js":42,"./net.js":50,"./perf_net.js":54,"./profiler_ui.js":58,"./settings.js":63,"./sprites.js":74,"./ui.js":78}],54:[function(require,module,exports){
"use strict"
...
},{"../common/wscommon":103,"./cmds":18,"./perf":53,"./settings":63}],55:[function(require,module,exports){
"use strict"
exports.enter=enter
exports.exit=exit
...
},{"../common/util.js":99}],56:[function(require,module,exports){
"use strict"
...
},{"./engine.js":23,"./local_storage.js":45,"assert":undefined}],58:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"../common/vmath.js":101,"./camera2d.js":15,"./cmds.js":18,"./engine.js":23,"./font.js":32,"./input.js":42,"./net.js":50,"./perf.js":53,"./profiler.js":57,"./settings.js":63,"./sprites.js":74,"./ui.js":78}],59:[function(require,module,exports){
"use strict"
...
exports.quat=quat
exports.unit_quat=void 0
var _require=require("../common/vmath.js"),rovec4=_require.rovec4,vec4=_require.vec4
var acos=Math.acos,cos=Math.cos,sin=Math.sin,sqrt=Math.sqrt
var EPSILON=1e-6
var unit_quat=rovec4(0,0,0,-1)
exports.unit_quat=unit_quat
...
q[1]*=il
q[2]*=il
q[3]*=il}qiNegW(q)}function qFromAxisAngle(out,axis,angle){var s=sin(angle*=.5)
out[0]=s*axis[0]
out[1]=s*axis[1]
out[2]=s*axis[2]
out[3]=cos(angle)
qiNegW(out)
return out}function qFromYPR(out,yaw,pitch,roll){var s_pitch=sin(.5*pitch)
var c_pitch=cos(.5*pitch)
var s_yaw=sin(.5*yaw)
var c_yaw=cos(.5*yaw)
var s_roll=sin(.5*roll)
var c_roll=cos(.5*roll)
...
qiNormalize(out)
return out}function qRotateX(out,a,rad){var ax=a[0]
var ay=a[1]
var az=a[2]
var aw=a[3]
var bx=sin(rad*=.5)
...
out[2]=az*bw-ay*bx
out[3]=aw*bw-ax*bx
return out}function qRotateY(out,a,rad){var ax=a[0]
var ay=a[1]
var az=a[2]
var aw=a[3]
var by=sin(rad*=.5)
...
out[2]=az*bw+ax*by
out[3]=aw*bw-ay*by
return out}function qRotateZ(out,a,rad){var ax=a[0]
var ay=a[1]
var az=a[2]
var aw=a[3]
var bz=sin(rad*=.5)
...
qiNormalize(v1)
var v0_t=(v0[0]*v1[0]+v0[1]*v1[1]+v0[2]*v1[2]+v0[3]*v1[3]<0?-1:1)*(1-t)
...
},{"../common/vmath.js":101}],60:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"../common/verify.js":100,"../common/vmath":101,"./camera2d.js":15,"./engine.js":23,"./input.js":42,"./spot.js":71,"./sprites.js":74,"./ui.js":78,"assert":undefined}],62:[function(require,module,exports){
"use strict"
...
function _inheritsLoose(subClass,superClass){subClass.prototype=Object.create(superClass.prototype)
_setPrototypeOf(subClass.prototype.constructor=subClass,superClass)}function _setPrototypeOf(o,p){return(_setPrototypeOf=Object.setPrototypeOf?Object.setPrototypeOf.bind():function _setPrototypeOf(o,p){o.__proto__=p
return o})(o,p)}exports.create=selectionBoxCreate
...
var _spritesJs=require("./sprites.js")
var spriteClipPause=_spritesJs.spriteClipPause
var spriteClipResume=_spritesJs.spriteClipResume
var spriteClipped=_spritesJs.spriteClipped
var _uiJs=require("./ui.js")
var playUISound=_uiJs.playUISound
var uiFontStyleFocused=_uiJs.uiFontStyleFocused
var uiFontStyleNormal=_uiJs.uiFontStyleNormal
...
function selboxDefaultDrawItemBackground(_ref){_ref.item_idx,_ref.item
var x=_ref.x,y=_ref.y,z=_ref.z,w=_ref.w,h=_ref.h,image_set=_ref.image_set,color=_ref.color,image_set_extra=_ref.image_set_extra,image_set_extra_alpha=_ref.image_set_extra_alpha
...
glov_ui.drawHBox({x:x,y:y,z:z+.001,w:w,h:h},image_set_extra,color_temp_fade)}}function selboxDefaultDrawItemText(_ref2){_ref2.item_idx
var item=_ref2.item,x=_ref2.x,y=_ref2.y,z=_ref2.z,w=_ref2.w,h=_ref2.h,display=_ref2.display,font_height=_ref2.font_height,style=_ref2.style
var text_z=z+1
var did_tab=false
if(display.tab_stop){var str=item.name
var tab_idx=str.indexOf("\t")
if(-1!==tab_idx){did_tab=true
...
var w2=w-display.tab_stop-2*display.xpad-pad
...
_md.w=w-2*display.xpad
_md.h=h
_md.style=style
var xx=x+display.xpad
if(display.use_markup)glov_markup.print(_md,xx,y,text_z,item.name)
else font.drawSizedAligned(_md.style,xx,y,text_z,_md.x_size,_md.align,_md.w,_md.h,item.name)}}function selboxDefaultDrawItem(param){selboxDefaultDrawItemBackground(param)
...
var COLORS=((_COLORS={})[SPOT_STATE_REGULAR]=color_white,_COLORS[SPOT_STATE_DOWN]=color_grayD0,_COLORS[SPOT_STATE_FOCUSED]=color_grayD0,_COLORS[SPOT_STATE_DISABLED]=color_gray80,_COLORS)
var SELBOX_BOUNCE_TIME=80
var GlovMenuItem=function GlovMenuItem(params){if((params=params||{})instanceof GlovMenuItem){for(var field in params)this[field]=params[field]
return}if("string"===typeof params)params={name:params}
this.name=params.name||"NO_NAME"
this.state=params.state||null
this.cb=params.cb||null
this.value=void 0===params.value?null:params.value
...
_proto.applyParams=function applyParams(params){if(!params)return
for(var f in params)if("items"===f)this.items=params.items.map(function(item){return new GlovMenuItem(item)})
else if("display"===f)merge(this.display,params[f])
else this[f]=params[f]}
_proto.isSelected=function isSelected(tag_or_index){if("number"===typeof tag_or_index)return this.selected===tag_or_index
...
else for(var ii=0;ii<this.items.length;++ii)if(!this.items[ii].disabled){this.selected=ii
break}}}
...
for(var ii=0;ii<this.items.length;++ii)if(!this.items[ii].disabled){if(-1===first_non_disabled_selection)first_non_disabled_selection=ii
num_non_disabled_selections++
last_non_disabled_selection=ii}var scroll_height=this.scroll_height
if(!scroll_height&&is_dropdown)scroll_height=camera2d.y1()-(y+entry_height)
...
if(!num_non_disabled_selections)this.selected=0
else if(idx>=last_non_disabled_selection)this.selected=last_non_disabled_selection
else if(idx<=first_non_disabled_selection)this.selected=first_non_disabled_selection
else{while(this.items[idx].disabled)idx+=delta
this.selected=idx}if(this.selected!==old_sel)if(list_visible)spotFocusSteal({key:key+"_"+this.selected})
...
if(!list_visible)page_size=1
if(page_size){var pad_shift=padButtonDown(PAD.RIGHT_TRIGGER)||padButtonDown(PAD.LEFT_TRIGGER)
var value=keyDownEdge(KEYS.PAGEDOWN)+(pad_shift?padButtonDownEdge(PAD.DOWN):0)
if(value){playUISound("rollover")
this.selectForward(this.selected+page_size*value)}if(value=keyDownEdge(KEYS.PAGEUP)+(pad_shift?padButtonDownEdge(PAD.UP):0)){playUISound("rollover")
...
x=y=0
eff_width=width-this.sa.barWidth()}else if(this.is_dropdown)spotSubBegin({key:key,x:x,y:y,z:z,w:width,h:scroll_height||entry_height})
var dt=glov_engine.getFrameDt()
...
if(item.href)link({x:x,y:y,w:width,h:entry_height,url:item.href})
var entry_spot_rect={def:SPOT_DEFAULT_BUTTON,key:key+"_"+ii,disabled:disabled||entry_disabled,disabled_focusable:false,x:x,y:y,w:width,h:entry_height,custom_nav:((_custom_nav={})[SPOT_NAV_RIGHT]=null,_custom_nav[SPOT_NAV_LEFT]=null,_custom_nav),auto_focus:item.auto_focus}
if(ii===first_non_disabled_selection&&this.nav_loop)entry_spot_rect.custom_nav[SPOT_NAV_UP]=key+"_"+last_non_disabled_selection
if(ii===last_non_disabled_selection&&this.nav_loop)entry_spot_rect.custom_nav[SPOT_NAV_DOWN]=key+"_"+first_non_disabled_selection
var entry_spot_ret=spot(entry_spot_rect)
...
this.was_right_clicked=2===entry_spot_ret.button
this.selected=ii
...
this.bounce_time=SELBOX_BOUNCE_TIME}else if(dt>=this.bounce_time)this.bounce_time=0
else{bounce=true
...
if(1===item.selection_alpha)image_set=glov_ui.sprites.menu_selected
else{image_set=glov_ui.sprites.menu_entry
...
if(1!==item.selection_alpha){image_set_extra=glov_ui.sprites.menu_selected
...
if(bounce)yoffs=-4*sin(20*this.bounce_time/SELBOX_BOUNCE_TIME/10)*entry_height/32
display.draw_item_cb({item_idx:ii,item:item,x:x,y:y+yoffs,z:z+1,w:eff_width,h:entry_height,image_set:image_set,color:color,image_set_extra:image_set_extra,image_set_extra_alpha:image_set_extra_alpha,font_height:font_height,display:display,style:style})
if(entry_spot_ret.ret&&item.href)window.location.href=item.href
y+=entry_height}ctx.scroll_area_consumed_click=false
...
assert(!params.is_dropdown,"Use dropDownCreate() instead");(_this=_SelectionBoxBase.call(this,params)||this).bounce_time=0
...
if(this.scroll_height&&this.items.length*entry_height>this.scroll_height)list_height=this.scroll_height
return(list_height+=2)+3}
...
if(ctx.any_focused)this.doPadMovement()
...
assert(!params.is_dropdown,"Old parameter: is_dropdown");(_this2=_SelectionBoxBase2.call(this,params)||this).dropdown_visible=false
_this2.last_selected=void 0
...
var root_spot_rect={key:key,disabled:disabled,x:x,y:y,z:z+2-.1,w:width,h:entry_height,def:SPOT_DEFAULT_BUTTON,custom_nav:((_custom_nav2={})[SPOT_NAV_RIGHT]=null,_custom_nav2)}
...
if(root_spot_ret.kb_focused||list_visible)this.doPadMovement()
this.selectForward(this.selected)
var eff_selection=this.dropdown_visible&&void 0!==this.last_selected?this.last_selected:this.selected
if(this.dropdown_visible&&(keyDownEdge(KEYS.ESC)||padButtonDownEdge(PAD.B))){this.selected=eff_selection
this.onListSelect()}if(root_spot_ret.ret||root_spot_ret.nav){if(root_spot_ret.nav)playUISound("button_click")
if(this.dropdown_visible){this.selected=eff_selection
...
font.drawSizedAligned(root_spot_ret.focused?uiFontStyleFocused():uiFontStyleNormal(),x+display.xpad,y,z+2,font_height,align,width-display.xpad-glov_ui.sprites.menu_header.uidata.wh[2]*entry_height,entry_height,this.items[eff_selection].name)
yret=(y+=entry_height)+2
if(this.dropdown_visible){z+=1e3
var clip_pause=spriteClipped()
if(clip_pause)spriteClipPause()
spotSubPush()
var do_scroll=scroll_height&&this.items.length*entry_height>scroll_height
if(!do_scroll&&y+this.items.length*entry_height>=camera2d.y1())y=camera2d.y1()-this.items.length*entry_height
this.doList(x,y,z,do_scroll,eff_selection)
if(this.was_clicked)this.dropdown_visible=false
spotSubPop()
if(clip_pause)spriteClipResume()
var any_focused=ctx.any_focused,scroll_area_consumed_click=ctx.scroll_area_consumed_click
if(root_spot_ret.ret||this.was_clicked)any_focused=true
this.is_focused=this.is_focused||any_focused
if(!any_focused)this.selected=eff_selection
if(!any_focused&&!root_spot_ret.focused&&!scroll_area_consumed_click)if(mouseButtonHadUpEdge()||spotPadMode()){this.selected=eff_selection
...
function selectionBoxCreate(params){if(!font)font=glov_ui.font
return new GlovSelectionBox(params)}function dropDownCreate(params){if(!font)font=glov_ui.font
return new GlovDropDown(params)}

},{"../common/util.js":99,"../common/vmath.js":101,"./camera2d.js":15,"./engine.js":23,"./font.js":32,"./input.js":42,"./link.js":44,"./scroll_area.js":61,"./spot.js":71,"./sprites.js":74,"./ui.js":78,"assert":undefined}],63:[function(require,module,exports){
"use strict"
exports.get=get
exports.pop=pop
exports.push=push
...
},{"../common/util.js":99,"./cmds.js":18,"./engine.js":23,"assert":undefined}],64:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"../common/vmath.js":101,"./camera2d.js":15,"./cmds.js":18,"./engine.js":23,"./fetch.js":30,"./font.js":32,"./input.js":42,"./scroll_area.js":61,"./settings.js":63,"./shaders.js":65,"./ui.js":78}],65:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"./engine.js":23,"./error_report.js":28,"./filewatch.js":31,"./textures.js":76,"./webfs.js":82,"assert":undefined}],66:[function(require,module,exports){
"use strict"
...
var GlovSimpleMenu=function(){function GlovSimpleMenu(params){this.sel_box=selectionBoxCreate(params=params||{})
this.edit_index=-1
...
var force=false
if(2===delta){delta=1
force=true}if(!menu_item.state){if(null!==menu_item.value&&(menu_item.prompt_int||menu_item.prompt_string))this.internal.edit_index=index
else if(null!==menu_item.value){if(keyDown(KEYS.SHIFT)&&!force)delta=-1
menu_item.value+=delta*menu_item.value_inc
if(menu_item.slider||menu_item.plus_minus)menu_item.value=clamp(menu_item.value,menu_item.value_min,menu_item.value_max)
else{if(menu_item.value<menu_item.value_min)menu_item.value=menu_item.value_max
if(menu_item.value>menu_item.value_max)menu_item.value=menu_item.value_min}}}else engine.setState(menu_item.state)
if(menu_item.cb)menu_item.cb()}
...
var y0=sel_box.y
var exit_index=-1
for(var i=0;i<items.length;++i)if(items[i].exit)exit_index=i
var selbox_enabled=!(params&&params.disabled)
...
if(menu_item.disabled)color=display.style_disabled.color
menu_item.value=slider(menu_item.value,{x:slider_x,y:y0+ii*sel_box.entry_height,z:z+3,w:slider_width,h:sel_box.entry_height,disabled:menu_item.disabled,color:color,min:menu_item.value_min,max:menu_item.value_max,pad_focusable:false})
if(sliderIsFocused())sel_box.show_as_focused=ii}else if(menu_item.plus_minus){assert("number"===typeof menu_item.value,"plus_minus items require a numerical value")
assert(menu_item.value_inc,"plus_minus items require a value increment")
var pad=6
var button_width=sel_box.entry_height
var button_x=x+sel_box.width-2*button_width-pad-4
var delta=0
if(ui.buttonText({pad_focusable:false,x:button_x,y:y0+ii*sel_box.entry_height,z:z+3,w:button_width,h:sel_box.entry_height,text:"-"}))delta=-1
var minus_over=ui.button_mouseover
if(ui.buttonText({pad_focusable:false,x:button_x+pad+button_width,y:y0+ii*sel_box.entry_height,z:z+3,w:button_width,h:sel_box.entry_height,text:"+"}))delta=1
var plus_over=ui.button_mouseover
...
y+=sel_box.run()
var selected=-1
if(-1!==exit_index&&(keyDownEdge(KEYS.ESC)||!items[exit_index].no_controller_exit&&padButtonDownEdge(PAD.CANCEL))){this.execItem(exit_index,1)
selected=exit_index}if(sel_box.was_clicked){this.execItem(sel_box.selected,sel_box.was_right_clicked?-1:1)
selected=sel_box.selected}if(-1!==(this.selected=selected)&&!items[selected].no_sound)if(items[selected].exit)ui.playUISound("cancel")
else ui.playUISound("select")
return y-y0}
_proto.isSelected=function isSelected(tag_or_index){if(-1===this.selected)return false
if(void 0===tag_or_index)return this.sel_box.items[this.selected].tag||true
...
},{"../common/util.js":99,"../common/vmath.js":101,"./camera2d.js":15,"./engine.js":23,"./input.js":42,"./selection_box.js":62,"./slider.js":67,"./ui.js":78,"assert":undefined}],67:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"../common/vmath.js":101,"./input.js":42,"./spot.js":71,"./ui.js":78,"assert":undefined}],68:[function(require,module,exports){
"use strict"
...
},{"../common/enums":92,"../common/friends_data":93,"../common/util":99,"./client_config":17,"./cmds":18,"./input":42,"./net":50,"./sprites":74,"./textures":76,"assert":undefined}],69:[function(require,module,exports){
"use strict"
...
},{"../common/util":99,"./browser":13,"./cmds":18,"./fbinstant":29,"./filewatch":31,"./settings":63,"./urlhash":80,"@jimbly/howler/src/howler.core.js":undefined,"assert":undefined}],70:[function(require,module,exports){
"use strict"
...
var v3addScale=_glovCommonVmath.v3addScale
var vec3=_glovCommonVmath.vec3
var zero_vec=_glovCommonVmath.zero_vec
...
var _dyn_geomJs=require("./dyn_geom.js")
var dynGeomAlloc=_dyn_geomJs.dynGeomAlloc
var dynGeomSpriteSetup=_dyn_geomJs.dynGeomSpriteSetup
var _spritesJs=require("./sprites.js")
var BLEND_ADDITIVE=_spritesJs.BLEND_ADDITIVE
var BLEND_ALPHA=_spritesJs.BLEND_ALPHA
var queueSpriteData=_spritesJs.queueSpriteData
var spriteDataAlloc=_spritesJs.spriteDataAlloc
...
var atlases={}
function spineLoadAtlas(filename){if(atlases[filename])return atlases[filename]
var atlas_text=webFSGetFile(filename,"text")
...
var cached=skeletons[filename]
if(!cached){var atlas_loader=new AtlasAttachmentLoader(atlas)
var _skeleton_data
if(filename.endsWith(".json"))assert(false)
else{var skeleton_binary=new SkeletonBinary(atlas_loader)
skeleton_binary.scale=1
_skeleton_data=skeleton_binary.readSkeletonData(webFSGetFile(filename,"binary"))}var _animation_state_data=cached?cached.animation_state_data:new AnimationStateData(_skeleton_data)
for(var from in mix){var map=mix[from]
for(var to in map){var v=map[to]
_animation_state_data.setMix(from,to,v)
if(!mix[to]||void 0===mix[to][from])_animation_state_data.setMix(to,from,v)}}cached=skeletons[filename]={skeleton_data:_skeleton_data,animation_state_data:_animation_state_data}}var _cached=cached,skeleton_data=_cached.skeleton_data,animation_state_data=_cached.animation_state_data
return{skeleton:new Skeleton(skeleton_data),animation_state_data:animation_state_data}}var tempPos=new Vector2
...
var _spineLoadSkeleton=spineLoadSkeleton(spineLoadAtlas(params.atlas),params),skeleton=_spineLoadSkeleton.skeleton,animation_state_data=_spineLoadSkeleton.animation_state_data
this.skeleton=skeleton
this.animation_state=new AnimationState(animation_state_data)
this.vertices=new Float32Array(1024)
if(anim)this.setAnimation(0,anim,true)}Spine.prototype.getAnimation=function(track_index){var cur=this.animation_state.getCurrent(track_index=track_index||0)
...
if(cur&&cur.animation&&cur.animation.name===name)return
...
var skeleton=this.skeleton,vertices=this.vertices
...
var buf=sprite.data
for(var corner=0,vout=0;corner<4;corner++){var vin=2*lookup[corner]
...
var numFloats=(numVertices=mesh.worldVerticesLength>>1)*clippedVertexSize
if(numFloats>vertices.length)vertices=this.vertices=new Float32Array(numFloats)
...
uvs=mesh.uvs
attachmentColor=mesh.color}else if(attachment instanceof ClippingAttachment)continue
else continue
if(!texture)continue
...
else{if(premultipliedAlpha){darkColor.r=slot.darkColor.r*finalColor.a
darkColor.g=slot.darkColor.g*finalColor.a
darkColor.b=slot.darkColor.b*finalColor.a}else darkColor.setFromColor(slot.darkColor)
darkColor.a=premultipliedAlpha?1:0}var slotBlendMode=slot.data.blendMode
blend=BLEND_ALPHA
if(slotBlendMode===BlendMode.Additive)blend=BLEND_ADDITIVE
var verts=vertices
if(this.vertexEffect){var vertexEffect=this.vertexEffect
for(var v=0;v<2*numVertices;v+=2){tempPos.x=verts[v]
tempPos.y=verts[v+1]
tempUv.x=uvs[v]
tempUv.y=uvs[v+1]
...
assert(false,"Not yet implemented")}}else if(4===numVertices){var sprite=spriteDataAlloc(texture.texs,null,null,blend)
var buf=sprite.data
for(var vin=0,vout=0;vin<2*numVertices;vin+=2){buf[vout++]=x+verts[vin]*scale
...
buf[vout++]=uvs[vin]
buf[vout++]=uvs[vin+1]}queueSpriteData(sprite,z)
z+=ZINC}else for(var tri_idx=0;tri_idx<triangles.length;){var ti_b=tri_idx+3
var num_match=match_b[0]=match_b[1]=match_b[2]=0
for(var ii=0;ii<3;++ii){match_a[ii]=-1
for(var jj=0;jj<3;++jj)if(triangles[tri_idx+ii]===triangles[ti_b+jj]){++num_match
match_a[ii]=jj
match_b[jj]=1
break}}if(2===num_match){var unmatchedA=-1===match_a[0]?0:-1===match_a[1]?1:2
...
tri_idx+=3}}}}
var pos0=vec3()
Spine.prototype.draw3D=function(param){var _dynGeomSpriteSetup=dynGeomSpriteSetup(param),bucket=_dynGeomSpriteSetup.bucket,my_right=_dynGeomSpriteSetup.my_right,my_down=_dynGeomSpriteSetup.my_down,sort_z=_dynGeomSpriteSetup.sort_z,shader=_dynGeomSpriteSetup.shader,vshader=_dynGeomSpriteSetup.vshader,shader_params=_dynGeomSpriteSetup.shader_params
var pos=param.pos,offs=param.offs,scale=param.scale,color=param.color
color=color||unit_vec
if(void 0===scale)scale=1
v3addScale(pos0,pos,my_right,(offs=offs||zero_vec)[0])
...
var uvs
var blend
for(var i=0;i<drawOrder.length;i++){var slot=drawOrder[i]
if(!slot.bone.active)continue
var attachment=slot.getAttachment()
var numVertices=void 0
var triangles=void 0
...
var verts=elem.verts,tris=elem.tris
assert(tris.length>=triangles.length)
assert.equal(3*elem.num_tris,triangles.length)
elem.shader=shader
elem.vshader=vshader
elem.shader_params=shader_params
elem.blend=blend
...
if(this.vertexEffect){var vertexEffect=this.vertexEffect
for(var v=0;v<2*numVertices;v+=2){tempPos.x=vertices[v]
...
tempDark.set(0,0,0,0)
vertexEffect.transform(tempPos,tempUv,tempLight,tempDark)
assert(false,"Not yet implemented")}}else{var vout=0
for(var vin=0;vin<2*numVertices;vin+=2){var x=vertices[vin]*scale
var y=-vertices[vin+1]*scale
...
verts[vout++]=0
verts[vout++]=0}assert(vout<=verts.length)}for(var tri_idx=0;tri_idx<triangles.length;tri_idx+=3){tris[tri_idx]=triangles[tri_idx+2]
tris[tri_idx+1]=triangles[tri_idx+1]
tris[tri_idx+2]=triangles[tri_idx]}elem.queue(bucket,sort_z)}}
function spineCreate(params){return new Spine(params)}

},{"../common/vmath":101,"./dyn_geom.js":20,"./sprites.js":74,"./textures.js":76,"./webfs.js":82,"assert":undefined,"spine-core/AnimationState":undefined,"spine-core/AnimationStateData":undefined,"spine-core/AtlasAttachmentLoader":undefined,"spine-core/ClippingAttachment":undefined,"spine-core/MeshAttachment":undefined,"spine-core/RegionAttachment":undefined,"spine-core/Skeleton":undefined,"spine-core/SkeletonBinary":undefined,"spine-core/SlotData":undefined,"spine-core/TextureAtlas":undefined,"spine-core/Utils":undefined}],71:[function(require,module,exports){
"use strict"
...
},{"../common/verify":100,"./camera2d.js":15,"./engine.js":23,"./font.js":32,"./input.js":42,"./input_constants":43,"./settings.js":63,"./ui.js":78,"assert":undefined}],72:[function(require,module,exports){
"use strict"
exports.spriteAnimationCreate=spriteAnimationCreate
...
if("number"===typeof anim.frames)anim.frames=[anim.frames]
if("number"===typeof anim.times){var arr=new Array(anim.frames.length)
for(var ii=0;ii<anim.frames.length;++ii)arr[ii]=anim.times
anim.times=arr}if(anim.times_random)if("number"===typeof anim.times_random){var _arr=new Array(anim.frames.length)
for(var _ii=0;_ii<anim.frames.length;++_ii)_arr[_ii]=anim.times_random
anim.times_random=_arr}for(var _ii2=anim.total_time=0;_ii2<anim.times.length;++_ii2)anim.total_time+=anim.times[_ii2]
if(void 0===anim.loop)anim.loop=true}}}function spriteAnimationCreate(params){return new GlovSpriteAnimation(params)}exports.createSpriteAnimation=spriteAnimationCreate
...
GlovSpriteAnimation.prototype.setState=function(state,force){if(state===this.state&&!force)return this
if(!this.data[state]){console.error("Tried to set anim state "+state+" which does not exist")
return this}this.state=state
this.anim=this.data[state]
if(this.anim.init_time)this.time=floor(random()*this.anim.init_time)
else this.time=0
this.setFrameIndex(0)
return this}
GlovSpriteAnimation.prototype.progress=function(){if(!this.anim)return 1
var time=this.time
for(var ii=0;ii<this.anim_idx;++ii)time+=this.anim.times[ii]
return time/this.anim.total_time}
GlovSpriteAnimation.prototype.update=function(dt){if(!this.anim)return
this.time+=dt
...
GlovSpriteAnimation.prototype.getFrame=function(dt){if(void 0!==dt)this.update(dt)
return this.frame}
...
},{"../common/util.js":99,"../common/vmath.js":101,"./camera2d.js":15,"./dyn_geom.js":20,"./engine.js":23,"./geom.js":34,"./shaders.js":65,"./textures.js":76,"assert":undefined}],75:[function(require,module,exports){
"use strict"
exports.create=create
var assert=require("assert")
var _glovClientClient_config=require("./client_config")
...
},{"../common/chunked_send":87,"../common/dot-prop":90,"../common/md5":94,"../common/packet":95,"../common/perfcounters":96,"../common/tiny-events":98,"../common/util":99,"./client_config":17,"./fbinstant":29,"./local_storage":45,"./net":50,"./walltime":81,"assert":undefined}],76:[function(require,module,exports){
"use strict"
exports.bind=bind
...
},{"../common/util.js":99,"./engine.js":23,"./filewatch.js":31,"./local_storage.js":45,"./settings.js":63,"./shaders.js":65,"./urlhash.js":80,"assert":undefined}],77:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"../common/verify.js":100,"../common/vmath.js":101,"./camera2d.js":15,"./effects.js":22,"./engine.js":23,"./framebuffer.js":33,"./shaders.js":65,"./sprites.js":74,"./textures.js":76,"./ui.js":78,"assert":undefined}],78:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"../common/vmath.js":101,"./camera2d.js":15,"./edit_box.js":21,"./effects.js":22,"./engine.js":23,"./font.js":32,"./input.js":42,"./link.js":44,"./localization.js":46,"./mat43.js":47,"./scroll_area.js":61,"./slider.js":67,"./sound.js":69,"./spot.js":71,"./sprites.js":74,"./textures.js":76,"assert":undefined}],79:[function(require,module,exports){
"use strict"
exports.run=run
...
var items=[]
for(var ii=0;ii<100;++ii)items.push("item"+ii)
test_dropdown_large=dropDownCreate({items:items,z:Z.UI,width:column_width-8})
...
if(demo_menu.isSelected()){if(demo_menu.isSelected("opt2"))demo_result="Selected the second option"
if(!demo_result)demo_result="Menu selected: "+demo_menu.getSelectedItem().name
demo_menu_up=false}ui.menuUp()
input.eatAllInput()}var pad=4
if(ui.buttonText({x:x,y:y,z:z,text:"Modal Dialog",tooltip:"Shows a modal dialog"})){demo_result=""
ui.modalDialog({title:"Modal Dialog",text:"This is a modal dialog!",buttons:{OK:function OK(){demo_result="OK pushed!"},Cancel:null}})}if(edit_box1.run()===edit_box1.SUBMIT)ui.modalDialog({title:"Modal Dialog",text:"Edit box submitted with text "+edit_box1.getText(),buttons:{OK:null}})
if(edit_box2.run()===edit_box2.SUBMIT)edit_box2.setText("")
if(ui.buttonText({x:edit_box2.x+edit_box2.w+pad,y:y,z:z,text:"...",w:ui.button_height}))ui.modalTextEntry({title:"Type something",edit_text:edit_box2.getText(),buttons:{ok:function ok(text){edit_box2.setText(text)},cancel:null}})
if(ui.buttonText({x:x,y:y+=line_height,z:z,text:"Menu",tooltip:"Shows a menu"}))demo_menu_up=true
colorPicker({x:x+column_width,y:y,z:z,color:test_color})
test_scroll_area.begin({x:x+column_width+4+line_height,y:y+4,z:z,w:100,h:8*ui.font_height+pad})
var internal_y=2
...
if(linkText({x:column_width/2,y:internal_y,text:"Int URL",internal:true,url:engine.defines.SPOT_DEBUG?getURLBase():getURLBase()+"?D=SPOT_DEBUG"}))engine.defines.SPOT_DEBUG=!engine.defines.SPOT_DEBUG
internal_y+=ui.font_height+pad
internal_y+=test_dropdown_large.run({x:2,y:internal_y,z:z+1})+pad
if(ui.buttonText({x:2,y:internal_y,z:z,text:"Disabled",tooltip:"A disabled button",disabled:true}))assert(false)
internal_y+=ui.button_height+pad
for(var ii=0;ii<test_lines;++ii){ui.print(font_style,2,internal_y,z+1,"Line #"+ii)
internal_y+=ui.font_height+pad}if(ui.buttonText({x:2,y:internal_y,z:z+1,text:"Add Line",key:"add_line"}))++test_lines
internal_y+=ui.button_height+pad
if(ui.buttonText({x:2,y:internal_y,z:z+1,text:"Remove Line",key:"remove_line"}))--test_lines
internal_y+=ui.button_height+pad
test_scroll_area.end(internal_y)
ui.panel({x:test_scroll_area.x-pad,y:test_scroll_area.y-pad,z:z-1,w:test_scroll_area.w+2*pad,h:test_scroll_area.h+2*pad})
y+=line_height
...
ui.progressBar({x:x+ui.button_width+2*pad+2*ui.font_height,y:y,z:z,w:60,h:ui.button_height,progress:slider_value})
ui.button_height}function runFontTest(x,y){var COLOR_RED=4278190335
var COLOR_GREEN=16711935
var COLOR_WHITE=4294967295
var COLOR_INVISIBLE=0
var z=Z.UI
var font=ui.font
var font_size=2*ui.font_height
font.drawSized(null,x,y,z,font_size,"Default "+font_size+" "+random().toFixed(7))
font.drawSized(null,x,y+=font_size,z,font_size/2,"Default "+font_size/2+" Lorem ipsum dolor sit amet <[A|B]>")
y+=ceil(font_size/2)
...
font.drawSizedAligned(font_style_both2,x,y,z,font_size2,ALIGN.HFIT,2*ui.button_width,0,"ALIGN.HFIT The quick brown fox jumps over the lazy dog")
y+=font_size2
var test="glow"
if("outline"===test){var font_style_outline2=fontStyle(null,{outline_width:1,outline_color:COLOR_RED,glow_xoffs:0,glow_yoffs:0,glow_inner:0,glow_outer:0,glow_color:COLOR_INVISIBLE,color:COLOR_WHITE})
for(var ii=1;ii<=4;ii++){font.drawSizedAligned(fontStyle(font_style_outline2,{outline_width:2*ii}),x,y,z,font_size2,ALIGN.HLEFT,400,0,"Outline thickness "+2*ii)
y+=font_size2}}else if("glow"===test){var font_style_glow2=fontStyle(null,{outline_width:0,outline_color:COLOR_INVISIBLE,glow_xoffs:0,glow_yoffs:0,glow_inner:0,glow_outer:8,glow_color:COLOR_RED,color:COLOR_WHITE})
for(var _ii=1;_ii<=4;_ii++){font.drawSizedAligned(fontStyle(font_style_glow2,{glow_outer:2*_ii}),x,y,z,font_size2,ALIGN.HLEFT,400,0,"Glow outer "+2*_ii)
y+=font_size2}}else if("wrap"===test)font.drawSizedWrapped(null,x,y,z,400,24,font_size2,"Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt utlaboreetdoloremagnaaliqua.")}

},{"../common/vmath":101,"./color_picker.js":19,"./edit_box":21,"./engine.js":23,"./font":32,"./input.js":42,"./link.js":44,"./scroll_area":61,"./selection_box":62,"./simple_menu":66,"./slider.js":67,"./ui.js":78,"./urlhash.js":80,"assert":undefined}],80:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"assert":undefined}],81:[function(require,module,exports){
"use strict"
...
},{"../common/util.js":99,"./filewatch.js":31,"./urlhash.js":80,"assert":undefined}],83:[function(require,module,exports){
"use strict"
...
},{"../../common/rand_alea.js":97,"../../common/words/profanity_common.js":102,"../rand_fast.js":60,"../webfs.js":82}],84:[function(require,module,exports){
"use strict"
...
function getVersionUrlParams(){return"plat="+PLATFORM+"&ver="+exports.CURRENT_VERSION+"&build="+"1673321730282"}function jsonParseResponse(response){if(!response)return null
...
return}}cb()})}doit()}WSClient.prototype.onBuildTimestamp=function(build_timestamp){if(build_timestamp!=="1673321730282")if(this.on_build_timestamp_mismatch)this.on_build_timestamp_mismatch()
else if(getAbilityReload()){console.error("App build mismatch (server: "+build_timestamp+", client: "+"1673321730282"+"), reloading")
whenServerReady(function(){if(window.reloadSafe)window.reloadSafe()
else document.location.reload()})}else console.warn("App build mismatch (server: "+build_timestamp+", client: "+"1673321730282"+"), ignoring")}
...
},{"../common/ack.js":85,"../common/perfcounters.js":96,"../common/wscommon.js":103,"./client_config.js":17,"./environments":27,"./error_report.js":28,"./fetch.js":30,"./urlhash.js":80,"assert":undefined}],85:[function(require,module,exports){
"use strict"
...
},{"./packet.js":95,"./perfcounters.js":96,"assert":undefined}],86:[function(require,module,exports){
...
},{"buffer":undefined}],87:[function(require,module,exports){
"use strict"
...
},{"./crc32.js":89,"./packet.js":95,"assert":undefined,"glov-async":undefined}],88:[function(require,module,exports){
"use strict"
...
},{"./perfcounters.js":96,"./util.js":99,"assert":undefined}],89:[function(require,module,exports){
"use strict"
...
},{"./util.js":99}],91:[function(require,module,exports){
"use strict"
exports.EntityUpdateCmd=exports.EntityFieldSub=exports.EntityFieldSpecial=exports.EntityFieldEncoding=exports.EntityBaseCommon=exports.EALF_HAS_PREDICATE=exports.EALF_HAS_PAYLOAD=exports.EALF_HAS_ENT_ID=exports.EALF_HAS_ASSIGNMENTS=void 0
var _EntityBaseCommon$reg
var assert=require("assert")
var EALF_HAS_PREDICATE=1
exports.EALF_HAS_PREDICATE=EALF_HAS_PREDICATE
var EALF_HAS_ENT_ID=2
exports.EALF_HAS_ENT_ID=EALF_HAS_ENT_ID
var EALF_HAS_PAYLOAD=4
exports.EALF_HAS_PAYLOAD=EALF_HAS_PAYLOAD
var EALF_HAS_ASSIGNMENTS=8
exports.EALF_HAS_ASSIGNMENTS=EALF_HAS_ASSIGNMENTS
var EntityFieldEncoding={JSON:1,Int:2,Float:3,AnsiString:4,U8:5,U32:6,String:7,Boolean:8,Vec2:9,Vec3:10,U8Vec3:11,IVec3:12,Custom0:127}
exports.EntityFieldEncoding=EntityFieldEncoding
var EntityFieldSub={None:0,Array:1,Record:2}
exports.EntityFieldSub=EntityFieldSub
var EntityFieldSpecial={Terminate:0,Default:1,MAX:2}
exports.EntityFieldSpecial=EntityFieldSpecial
var EntityUpdateCmd;(function(EntityUpdateCmd){EntityUpdateCmd[EntityUpdateCmd["Terminate"]=0]="Terminate"
EntityUpdateCmd[EntityUpdateCmd["Full"]=1]="Full"
EntityUpdateCmd[EntityUpdateCmd["Diff"]=2]="Diff"
EntityUpdateCmd[EntityUpdateCmd["Delete"]=3]="Delete"
EntityUpdateCmd[EntityUpdateCmd["Event"]=4]="Event"
EntityUpdateCmd[EntityUpdateCmd["Schema"]=5]="Schema"
EntityUpdateCmd[EntityUpdateCmd["IsInitialList"]=6]="IsInitialList"})((exports.EntityUpdateCmd=EntityUpdateCmd)||(exports.EntityUpdateCmd=EntityUpdateCmd={}))
var EntityBaseCommon=function(){function EntityBaseCommon(ent_id,entity_manager){this.id=void 0
this.data=void 0
this.entity_manager=void 0
this.id=ent_id
this.data={}
this.entity_manager=entity_manager}EntityBaseCommon.prototype.getData=function getData(field){assert(0)
return}
EntityBaseCommon.registerFieldEncoding=function registerFieldEncoding(data){for(var key_string in data){var key=Number(key_string)
var pair=data[key]
...
return EntityBaseCommon}();(exports.EntityBaseCommon=EntityBaseCommon).field_encoders={}
EntityBaseCommon.field_decoders={}
EntityBaseCommon.registerFieldEncoding(((_EntityBaseCommon$reg={})[EntityFieldEncoding.JSON]={encoder:function encJSON(ent,pak,value){pak.writeJSON(value)},decoder:function decJSON(pak,old_value){return pak.readJSON()}},_EntityBaseCommon$reg[EntityFieldEncoding.Int]={encoder:function encInt(ent,pak,value){pak.writeInt(value)},decoder:function decInt(pak,old_value){return pak.readInt()}},_EntityBaseCommon$reg[EntityFieldEncoding.Float]={encoder:function encFloat(ent,pak,value){pak.writeFloat(value)},decoder:function decFloat(pak,old_value){return pak.readFloat()}},_EntityBaseCommon$reg[EntityFieldEncoding.AnsiString]={encoder:function encAnsiString(ent,pak,value){pak.writeAnsiString(value)},decoder:function decAnsiString(pak,old_value){return pak.readAnsiString()}},_EntityBaseCommon$reg[EntityFieldEncoding.U8]={encoder:function encU8(ent,pak,value){pak.writeU8(value)},decoder:function decU8(pak,old_value){return pak.readU8()}},_EntityBaseCommon$reg[EntityFieldEncoding.U32]={encoder:function encU32(ent,pak,value){pak.writeU32(value)},decoder:function decU32(pak,old_value){return pak.readU32()}},_EntityBaseCommon$reg[EntityFieldEncoding.String]={encoder:function encString(ent,pak,value){pak.writeString(value)},decoder:function decString(pak,old_value){return pak.readString()}},_EntityBaseCommon$reg[EntityFieldEncoding.Boolean]={encoder:function encBool(ent,pak,value){pak.writeBool(value)},decoder:function decBool(pak,old_value){return pak.readBool()}},_EntityBaseCommon$reg[EntityFieldEncoding.Vec2]={encoder:function encVec2(ent,pak,value){var v=value
pak.writeFloat(v[0])
pak.writeFloat(v[1])},decoder:function decVec2(pak,old_value){var v=old_value||[]
v[0]=pak.readFloat()
v[1]=pak.readFloat()
return v}},_EntityBaseCommon$reg[EntityFieldEncoding.Vec3]={encoder:function encVec3(ent,pak,value){var v=value
pak.writeFloat(v[0])
pak.writeFloat(v[1])
pak.writeFloat(v[2])},decoder:function decVec3(pak,old_value){var v=old_value||[]
v[0]=pak.readFloat()
v[1]=pak.readFloat()
v[2]=pak.readFloat()
return v}},_EntityBaseCommon$reg[EntityFieldEncoding.U8Vec3]={encoder:function encU8Vec3(ent,pak,value){var v=value
pak.writeU8(v[0])
pak.writeU8(v[1])
pak.writeU8(v[2])},decoder:function decU8Vec3(pak,old_value){var v=old_value||[]
v[0]=pak.readU8()
v[1]=pak.readU8()
v[2]=pak.readU8()
return v}},_EntityBaseCommon$reg[EntityFieldEncoding.IVec3]={encoder:function encIVec3(ent,pak,value){var v=value
pak.writeInt(v[0])
pak.writeInt(v[1])
pak.writeInt(v[2])},decoder:function decIVec3(pak,old_value){var v=old_value||[]
v[0]=pak.readInt()
v[1]=pak.readInt()
v[2]=pak.readInt()
return v}},_EntityBaseCommon$reg))

},{"assert":undefined}],92:[function(require,module,exports){
"use strict"
...
},{"assert":undefined}],95:[function(require,module,exports){
(function (Buffer){(function (){
"use strict"
...
},{"./base64.js":86,"./util.js":99,"assert":undefined,"buffer":undefined}],96:[function(require,module,exports){
"use strict"
...
},{"assert":undefined}],99:[function(require,module,exports){
"use strict"
...
},{"gl-mat3/create":undefined,"gl-mat4/create":undefined}],102:[function(require,module,exports){
"use strict"
...
var trans_lookup={}
function canonize(str){return str.split("").map(function(c){return trans_lookup[c]||c}).join("")}function rot13(str){return str.split("").map(function(c){if((c=c.charCodeAt(0))>=97&&c<=122)c=97+(c-97+13)%26
...
},{"assert":undefined}],103:[function(require,module,exports){
"use strict"
...
},{"./ack.js":85,"./packet.js":95,"./perfcounters.js":96,"assert":undefined}]},{},[2])
